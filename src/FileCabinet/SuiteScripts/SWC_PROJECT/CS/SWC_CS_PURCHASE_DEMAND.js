 ...
 * @author Victor
 * @description 监听采购需求汇总表，供应商变化时自动更新BOM
 */
define(['../common/clientMask', 'N/https', 'N/ui/dialog', 'N/url', 'N/currentRecord', 'N/search', 'N/http', 'N/runtime'],
    function (Mask, https, dialog, url, currentRecord, search, http, runtime) {
        ...
        function fieldChanged(scriptContext) {
            if (scriptContext.fieldId == 'custrecord_swc_pd_vendor') {
                var newRec = scriptContext.currentRecord;
                var sku = newRec.getValue('custrecord_swc_pd_sku')
                var vendor = newRec.getValue('custrecord_swc_pd_vendor')
                setTimeout(function () {
                    vendor && search.create({
                        ...
                        newRec.setValue({ fieldId: 'custrecord_swc_pd_bom', value: bom_rec.id })
                        return false;
                    })
                }, 800)
