...
define(['N/currentRecord', 'N/https', 'N/record', 'N/search', 'N/url',"../APP/SWC_APP_HeardPayPlatform",'../common/MatchTool','N/ui/dialog'],
    ...
    function(currentRecord, https, record, search, url,app,MatchTool,dialog) {
        ...
        function pageInit(scriptContext) {
            ...
            var labelSpan = document.querySelector('#custpage_reconciliation_date_fs_lbl');
            if (labelSpan) {
                var aTag = labelSpan.querySelector('a');
                if (aTag) {
                    aTag.innerHTML = aTag.innerHTML.replace(/\*/g, '<span style="color:red;">*</span>');
                ...
                if (checkbox) {
                    if (!gcFlag || gcFlag == gcKey) {
                        ...
                        lineDataObj["po"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_sublist_cgdd_hide"});//补充

                        //对账单
                        lineDataObj["dzd"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_sublist_dzd_hide"});
                        ...
                        lineDataObj["item"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_fee_type_hide"});

                        //费用类型2
                        lineDataObj["itemName"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_fee_type_hide2"});
                        //数量
                        ...
                        //物流发运单号
                        lineDataObj["wlid"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_wl_id_hide"});//补充

                        //起运港
                        lineDataObj["qyg"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_wl_rm_sta_gk_hide"});

                        //目的港
                        lineDataObj["mdg"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_wl_md_lc_hide"});

                        //装柜日期
                        lineDataObj["zgrq"] = curRec.getCurrentSublistText(
                            {sublistId, fieldId: "custpage_swc_wl_loading_date_hide"});

                        //预计进港时间
                        lineDataObj["jgrq"] = curRec.getCurrentSublistText(
                            {sublistId, fieldId: "custpage_swc_wl_no_hide"});

                        //到港时间
                        lineDataObj["dgrq"] = curRec.getCurrentSublistText(
                            {sublistId, fieldId: "custpage_swc_wl_etd_hide"});

                        //出港待送货时间
                        lineDataObj["cgrq"] = curRec.getCurrentSublistText(
                            {sublistId, fieldId: "custpage_swc_gate_out_time_for_hide"});

                        //还柜时间
                        lineDataObj["hgrq"] = curRec.getCurrentSublistText(
                            {sublistId, fieldId: "custpage_swc_wl_return_time_hide"});

                        //SPO号
                        lineDataObj["spo"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_wl_spo_hide"});

                        // //柜号
                        lineDataObj["container"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_wl_container_number_hide"});

                        //总体积
                        lineDataObj["volume"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_wl_total_volume_hide"});

                        //合约柜/非合约柜
                        lineDataObj["cabinet"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_contract_cabinet1_hide"});

                        //全链路/到港
                        lineDataObj["full"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_fy_full_link_hide"});

                        //Broker Reference
                        lineDataObj["broker"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_broker_reference_hide"});//补充

                        //Entry Number
                        lineDataObj["entry"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_enter_number_hide"});//补充

                        //INV NO
                        lineDataObj["invno"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_inv_no_hide"});//补充

                        //海外仓入库单号
                        lineDataObj["hwrkdh"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_hw_lc_number_dz_hide"});

                        //总真实发运数量
                        lineDataObj["zzsfysl"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_total_shipment_qua_hide"});

                        //目的海外仓库代码
                        lineDataObj["mdhwckdm"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_md_location_hide"});

                        //货柜尺寸
                        lineDataObj["hgcc"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_wl_zg_size_hide"});

                        //调拨费-单据编号
                        lineDataObj["dbddjbh"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_document_number_hide"});

                        //尾程费用_出库单号
                        lineDataObj["wcckdh"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_lastmile_po_hide"});

                        //尾程费用_跟踪号
                        lineDataObj["wcgzh"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_lastmile_track_hide"});

                        //尾程费用_实物SKU/仓库SKU
                        lineDataObj["wcswsku"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_lastmile_sku_hide"});

                        //尾程费用_仓库代码
                        lineDataObj["wcckdm"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_lastmile_place_hide"});

                        //尾程费用_SKU数量
                        lineDataObj["wcquantity"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_lastmile_quantity_hide"});

                        //尾程费用_计费日期
                        lineDataObj["wcjfdate"] = curRec.getCurrentSublistText(
                            {sublistId, fieldId: "custpage_swc_lastmile_jfdate_hide"});

                        //尾程费用_入库单号/物流子单号
                        lineDataObj["wcrkdh"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_lastmile_receipt_hide"});
        ...
        function saveRecord(scriptContext) {
            ...
            // var bussiness = rec.getValue({
            //     fieldId: 'custpage_bussiness',
            // });
            ...
            if (gcFlag) {
                for (let i = 0; i < count; i++) {
                    ...
                    if (gcKey == gcFlag) {
                        ...
                        amountHS = amountHS + lineDataObj["grossamount"];
        ...
        async function createOrder() {
            ...
            try {
                ...
                if (result) {
                    ...
                    let vendorMain = curRec.getValue({fieldId: 'custpage_vendor'});
                    ...
                    poData["memo"] = memo;//备注
                    poData["amountsum"] = reconciliationAmount;//对账总金额
                    // poData["payAmount"] = payAmount;//应付总金额
                    poData["bussiness"] = bussiness;//提出部门
                    poData["account"] = account;
                    poData["terms"] = terms;
                    ...
                    if (poData["lineData"].length <= 0) {
                        ...
                        try {
                            let reqUrl = url.resolveScript({
                                scriptId: "customscript_swc_sl_heardpayplatform",
                                deploymentId: "customdeploy_swc_sl_heardpayplatform",
                                params: {
                ...
                if (data2.code === 200) {
                    ...
                    if (status === "COMPLETE") {
                        ...
                        if (error) {
                            ...
                            // errorRec.save();
                            clearInterval(poll);
                            alert(`客户付款申请单生成失败,请联系管理员: ${error}`);
                            document.getElementById('timeoutblocker').style.display = 'none';
                            // 刷新页面或跳转
                            window.onbeforeunload = null;
                            window.location.reload();
