 ...
 * @NModuleScope public
 */
define(['N/ui/dialog', 'N/https', 'N/url', 'N/record', 'N/search', 'N/currentRecord', 'N/url', '../common/SWC_CONFIG_DATA'],
    function (dialog, https, url, record, search, currentRecord, urls, SWC_CONFIG_DATA) {
        ...
        var SUITELET_INFO = {
            scriptId: CONFIG.SCRIPT_ID_SL_WL_PLAN_ORDER,
            deploymentId: CONFIG.DEPLOY_ID_SL_WL_PLAN_ORDER
        };
        var TRADE_TERMS_V2_SUITELET_INFO = {
            scriptId: CONFIG.SCRIPT_ID_SL_TRADE_TERMS_V2,
            deploymentId: CONFIG.DEPLOY_ID_SL_TRADE_TERMS_V2
        };
        ...
        function getTradeTermsV2SuiteletLink() {
            return url.resolveScript(TRADE_TERMS_V2_SUITELET_INFO);
        }
        ...
        function triggerTradeTermsV2Button() {
            confirmAndPostTradeTermsV2(false);
        }

        function autoExecuteTradeTermsV2() {
            var ctx = getTradeTermsV2Context();
            if (!ctx || ctx.mode !== 'auto' || !ctx.currentStepRows || !ctx.currentStepRows.length) {
                return;
            }
            confirmAndPostTradeTermsV2(true);
        }

        function confirmAndPostTradeTermsV2(isAuto) {
            var ctx = getTradeTermsV2Context();
            if (!ctx || !ctx.recId) return;

            var pendingKey = 'trade_terms_v2::' + String(ctx.recId || '') + '::' + String(ctx.currentStep || '');
            if (PENDING_REQUEST_MAP[pendingKey]) return;

            function doSubmit() {
                PENDING_REQUEST_MAP[pendingKey] = true;

                submitTradeTermsV2(ctx, isAuto).then(function (resp) {
                    var resultData = {};
                    try {
                        resultData = JSON.parse(resp.body || '{}');
                    } catch (e) {
                        resultData = { success: false, message: resp.body || '返回数据解析失败' };
                    }

                    if (!resultData.success || !resultData.taskId) {
                        delete PENDING_REQUEST_MAP[pendingKey];
                        return showAlertMessage(resultData.message || '任务提交失败');
                    }

                    startTradeTermsV2Overlay();
                    startTradeTermsV2Polling(resultData.taskId, pendingKey);
                }).catch(function (e) {
                    delete PENDING_REQUEST_MAP[pendingKey];
                    return showAlertMessage((e && e.message) ? e.message : '提交失败');
                });
            }
        ...
        function submitTradeTermsV2(ctx, isAuto) {
            ...
            console.log('trade terms v2 cs payload', {
                recId: String(ctx.recId || ''),
                currentStep: String(ctx.currentStep || ''),
                configId: String(ctx.configId || ''),
                configName: String(ctx.configName || ''),
                buttonText: String(ctx.buttonText || ctx.buttonLabel || ''),
                flowType: String(ctx.flowType || ''),
                routeBranch: String(ctx.routeBranch || '')
            });

            var requestData = {
                bill_id: String(ctx.recId),
                action: 'submit',
                payload: {
                    recId: String(ctx.recId),
                    currentStep: String(ctx.currentStep || ''),
                    ...
                    configId: String(ctx.configId || ''),
                    configName: String(ctx.configName || ''),
                    buttonId: String(ctx.buttonId || ''),
                    buttonText: String(ctx.buttonText || ctx.buttonLabel || ''),
                    documentTypeId: String(ctx.documentTypeId || ''),
                    documentTypeText: String(ctx.documentTypeText || ''),
                    startingWarehouseAttribute: String(ctx.startingWarehouseAttribute || ''),
                    warehouseAttribute: String(ctx.warehouseAttribute || ''),
            ...
            return https.post.promise({
                url: getTradeTermsV2SuiteletLink(),
                body: JSON.stringify(requestData),
                headers: JSON_HEADERS
            });
        }
        ...
        function startTradeTermsV2Polling(taskId, pendingKey) {
            stopTradeTermsV2Polling();
            setTradeTermsV2Progress(12, '任务已提交，正在后台处理中...');

            TRADE_TERMS_V2_PROGRESS_TIMER = window.setInterval(function () {
                var current = getTradeTermsV2Progress();
                if (current < 88) {
                    setTradeTermsV2Progress(current + randomTradeTermsV2Step(), '正在执行，请勿关闭页面...');
                }
            }, 1200);

            TRADE_TERMS_V2_STATUS_TIMER = window.setInterval(function () {
                queryTradeTermsV2TaskStatus(taskId).then(function (resp) {
                    var resultData = {};
                    try {
                        resultData = JSON.parse(resp.body || '{}');
                    } catch (e) {
                    ...
                    if (!resultData.success) {
                        stopTradeTermsV2Polling();
                        delete PENDING_REQUEST_MAP[pendingKey];
                        hideTradeTermsV2Overlay();
                        return showAlertMessage(resultData.message || '任务状态查询失败');
                    }

                    if (resultData.isDone) {
                        stopTradeTermsV2Polling();
                        delete PENDING_REQUEST_MAP[pendingKey];
                        setTradeTermsV2Progress(100, '执行完成，正在刷新页面...');
                        window.setTimeout(function () {
                            hideTradeTermsV2Overlay();
                            showAlertMessage('执行完成').then(function () {
                                window.location.reload();
                            });
                        }, 500);
                        return;
                    ...
                    if (resultData.isFailed) {
                        ...
                        delete PENDING_REQUEST_MAP[pendingKey];
                        hideTradeTermsV2Overlay();
                        return showAlertMessage('任务执行失败，请刷新页面确认结果。');
                    }
                }).catch(function () {
                });
            }, 3000);
        ...
        function stopTradeTermsV2Polling() {
            if (TRADE_TERMS_V2_PROGRESS_TIMER) {
                window.clearInterval(TRADE_TERMS_V2_PROGRESS_TIMER);
                TRADE_TERMS_V2_PROGRESS_TIMER = null;
            }
            if (TRADE_TERMS_V2_STATUS_TIMER) {
                window.clearInterval(TRADE_TERMS_V2_STATUS_TIMER);
                TRADE_TERMS_V2_STATUS_TIMER = null;
            }
        }

        function randomTradeTermsV2Step() {
            return Math.floor(Math.random() * 8) + 3;
        }

        function getTradeTermsV2Progress() {
            var bar = document.getElementById('swc_trade_terms_v2_progress_bar');
            if (!bar) return 0;
            var width = parseInt(String(bar.getAttribute('data-progress') || '0'), 10);
            return isNaN(width) ? 0 : width;
        }

        function setTradeTermsV2Progress(progress, text) {
            var overlay = ensureTradeTermsV2Overlay();
            var bar = overlay.querySelector('#swc_trade_terms_v2_progress_bar');
            var label = overlay.querySelector('#swc_trade_terms_v2_progress_label');
            var textNode = overlay.querySelector('#swc_trade_terms_v2_progress_text');
            var safeProgress = Math.max(0, Math.min(100, parseInt(progress, 10) || 0));

            if (bar) {
                bar.style.width = safeProgress + '%';
                bar.setAttribute('data-progress', String(safeProgress));
            }
            if (label) {
                label.innerHTML = safeProgress + '%';
            }
            if (textNode && text) {
                textNode.innerHTML = String(text);
            }
        }

        function startTradeTermsV2Overlay() {
            var overlay = ensureTradeTermsV2Overlay();
            overlay.style.display = 'flex';
            setTradeTermsV2Progress(8, '正在提交后台任务...');
        }

        function hideTradeTermsV2Overlay() {
            stopTradeTermsV2Polling();
            var overlay = document.getElementById('swc_trade_terms_v2_overlay');
            if (overlay) {
                overlay.style.display = 'none';
            }
        }

        function ensureTradeTermsV2Overlay() {
            var overlay = document.getElementById('swc_trade_terms_v2_overlay');
            if (overlay) return overlay;

            var html =
                '<div id="swc_trade_terms_v2_overlay" style="display:flex;position:fixed;inset:0;background:rgba(7,14,24,0.55);z-index:99999;align-items:center;justify-content:center;">' +
                '<div style="width:460px;max-width:92vw;background:#fff;border-radius:14px;padding:24px 24px 20px;box-shadow:0 18px 40px rgba(15,23,42,0.22);">' +
                '<div style="font-size:18px;font-weight:700;color:#14213d;margin-bottom:8px;">贸易条款处理中</div>' +
                '<div id="swc_trade_terms_v2_progress_text" style="font-size:13px;color:#4a5568;margin-bottom:16px;">正在准备任务...</div>' +
                '<div style="height:12px;background:#e8edf3;border-radius:999px;overflow:hidden;">' +
                '<div id="swc_trade_terms_v2_progress_bar" data-progress="0" style="height:12px;width:0%;background:linear-gradient(90deg,#0f766e,#14b8a6);transition:width 0.5s ease;"></div>' +
                '</div>' +
                '<div id="swc_trade_terms_v2_progress_label" style="margin-top:10px;text-align:right;font-size:12px;color:#0f766e;font-weight:700;">0%</div>' +
                '</div>' +
                '</div>';

            insertHTML(document.body, 'beforeend', html);
            return document.getElementById('swc_trade_terms_v2_overlay');
        }

        function getTradeTermsV2Context() {
