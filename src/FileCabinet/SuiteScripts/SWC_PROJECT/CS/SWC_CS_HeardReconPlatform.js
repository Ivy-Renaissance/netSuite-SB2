...
define(['N/currentRecord', 'N/https', 'N/record', 'N/search', 'N/url',"../APP/SWC_APP_HeardReconPlatform",'../common/MatchTool','N/ui/dialog'],
    ...
    function(currentRecord, https, record, search, url,app,MatchTool,dialog) {
        ...
        function pageInit(scriptContext) {
            ...
            var labelSpan = document.querySelector('#custpage_xxdzd_fs_lbl');
            if (labelSpan) {
                var aTag = labelSpan.querySelector('a');
                if (aTag) {
                    aTag.innerHTML = aTag.innerHTML.replace(/\*/g, '<span style="color:red;">*</span>');
                ...
                if (checkbox) {
                    if (!gcFlag || gcFlag == gcKey) {
                        ...
                        lineDataObj["po"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_sublist_cgdd_hide"});

                        //单据ID
                        lineDataObj["id"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_sublist_documentid_hide"});
                        ...
                        //费用类型2
                        lineDataObj["item2"] = curRec.getCurrentSublistValue(
                        ...
                        lineDataObj["wlid"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_wl_id_hide"});
                        ...
                        lineDataObj["broker"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_broker_reference_hide"});

                        //Entry Number
                        lineDataObj["entry"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_enter_number_hide"});

                        //INV NO
                        lineDataObj["invno"] = curRec.getCurrentSublistValue(
                            {sublistId, fieldId: "custpage_swc_inv_no_hide"});
        ...
        function saveRecord(scriptContext) {
            ...
            // });
            var xxdzd = rec.getValue({
                fieldId: 'custpage_xxdzd',
            });
            ...
            if (gcFlag) {
                for (let i = 0; i < count; i++) {
                    ...
                    if (gcKey == gcFlag) {
                        ...
                        amountHS = amountHS +  lineDataObj["amount"];
                    ...
                    if (poData["lineData"].length <= 0) {
                        ...
                        try {
                            let reqUrl = url.resolveScript({
                                scriptId: "customscript_swc_sl_heardreconplatform",
                                deploymentId: "customdeploy_swc_sl_heardreconplatform",
                                params: {
