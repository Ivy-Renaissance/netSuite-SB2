...
define(["N/search", "../common/SWC_Utils.js"],
    ...
    (search, SWC_Utils) => {
        ...
        const beforeSubmit = (scriptContext) => {
            ...
            let item = curRec.getValue({fieldId: "custrecord_swc_ys_item"}); // 货品
            if (SWC_Utils.isEmpty(item)) return;
            let description = curRec.getValue({fieldId: "custrecord_swc_ys_sbys"}); // 申报要素
            if (!SWC_Utils.isEmpty(description)) return; // 申报要素 已有值 则不更新
            let skuDescription = getSkuDescription(item);
            curRec.setValue({fieldId: "custrecord_swc_ys_sbys", value: skuDescription}); // 申报要素
        }
        ...
        function getSkuDescription(item) {
            ...
            if (SWC_Utils.isEmpty(itemInfo)) return skuDescription;
