 ...
 * @description 提货计划特殊修改时修改PMC审批备货
 */
define(['N/search', '../common/moment'],
    /**
 * @param{search} search
 */
    (search, moment) => {
        ...
        const beforeSubmit = (scriptContext) => {
            ...
            if (scriptContext.type == 'create') {
                var country = demandRec.getValue('custrecord_swc_sdp_country');//国家编码
                var store = demandRec.getValue('custrecord_swc_sdp_store');//平台（店铺）
                var locationType = demandRec.getValue('custrecord_swc_sdp_location_type');//仓库类型
                var sku = demandRec.getValue('custrecord_swc_sdp_sku');
                var level = getSKULevel(sku, country);
                if (level) {
                    demandRec.setValue({ fieldId: 'custrecord_swc_sdp_sku_level', value: level.level })//产品等级  
                }
                demandRec.setValue({ fieldId: 'custrecord_swc_sdp_batch_month', value: "TH-" + moment().add(16, 'hours').startOf('month').format('YYYYMM') }) //提货月批次编码
                // demandRec.setValue({ fieldId: 'custrecord_swc_sdp_months', value: 6 })//预测的总月份，默认6个月，可配置8个月
                var first3MUnits = 0; //前三个月需求
                var forcast_effective = 0; //运营周期内有效需求
                //开始搜索销售预测数量
                var unit_m1 = demandRec.getValue('custrecord_swc_sdp_forcast_m1');//第1月需求
                var unit_m2 = demandRec.getValue('custrecord_swc_sdp_forcast_m1');//第2月需求
                var unit_m3 = demandRec.getValue('custrecord_swc_sdp_forcast_m1');//第3月需求
                var unit_m4 = demandRec.getValue('custrecord_swc_sdp_forcast_m1');//第4月需求
                var unit_m5 = demandRec.getValue('custrecord_swc_sdp_forcast_m1');//第5月需求
                var unit_m6 = demandRec.getValue('custrecord_swc_sdp_forcast_m1');//第6月需求
                var unit_m7 = demandRec.getValue('custrecord_swc_sdp_forcast_m1');//第7月需求
                var unit_m8 = demandRec.getValue('custrecord_swc_sdp_forcast_m1');//第8月需求
                ...
                demandRec.setValue({ fieldId: 'custrecord_swc_sdp_old_logic_safety_stock', value: oldSafeQty })
                ...
                demandRec.setValue({ fieldId: 'custrecord_swc_sdp_new_logic_safety_stock', value: newSafeQty })//新逻辑-安全库存
                //预估总数
            ...
            var special_modification = demandRec.getValue('custrecord_swc_sdp_special_modification');//特殊修改
            if (special_modification !== '') {
                demandRec.setValue({ fieldId: 'custrecord_swc_sdp_quantity', value: special_modification })//PMC审批数量
            }
