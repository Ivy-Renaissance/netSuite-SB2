    ...
    function (query, search, format, record,runtime,url,MatchTool,SWC_CONFIG_DATA) {
        ...
        function queryData(parameters,pageSize,pageId) {
            ...
            if (parameters.custpage_dzd) {
                let dzdData = String(parameters.custpage_dzd)
                .split(/[^0-9]+/)
                .filter(id => id.length > 0);
                filter.push("AND");
                filter.push(["custcol_swc_vendor_statement","anyof", dzdData]);
            }
            ...
            var transactionSearchObj = search.create({
                ...
                columns:
                    [
                        ...
                        search.createColumn({
                            name: "custrecord_swc_wl_container_number",
                            join: "CUSTBODY_SWC_WL_NO",
                            label: "集装箱箱号"
                        }),
            ...
            if(allDataCount > 0) {
                ...
                results.forEach((result,index)=> {
                    ...
                    var key = custpage_sublist_documentid + '_' + custpage_sublist_lineid;
                    // if (paidObj.indexOf(key) != -1)
                    //     return;
                    ...
                    var custpage_sublist_type_hide = result.getValue({name: "type", label: "类型"});
                    ...
                    var  custpage_swc_fee_type_hide = result.getValue({name: "item", label: "货品"}) || "";
                    var  custpage_swc_fee_type = result.getText({name: "item", label: "货品"}) || "";
                    var  custpage_swc_fee_type_hide2 = result.getValue({
                        name: "displayname",
                        join: "item",
                        label: "显示名称"
                    }) || "";
                    var  custpage_sublist_dzd = result.getText({name: "custcol_swc_vendor_statement", label: "对账单"}) || "";
                    var  custpage_sublist_dzd_hide = result.getValue({name: "custcol_swc_vendor_statement", label: "对账单"}) || "";
                    ...
                    item.custpage_sublist_dzd = custpage_sublist_dzd;
                    item.custpage_sublist_dzd_hide = custpage_sublist_dzd_hide;
                    ...
                    item.custpage_sublist_type_hide = custpage_sublist_type_hide;
            ...
            //         var custpage_sublist_documentid = result.getValue({name: "internalid", label: "内部 ID"}) || "";
            //         var custpage_sublist_lineid = result.getValue({name: "line", label: "行 Id"}) || "";
            //         var custpage_sublist_subsidiary_hide = result.getValue({name: "subsidiarynohierarchy", label: "子公司（无层次结构）"}) || "";
            //         var custpage_sublist_subsidiary = result.getText({name: "subsidiarynohierarchy", label: "子公司（无层次结构）"}) || "";
            //         var  custpage_sublist_vendor_hide = result.getValue({
            ...
            var customrecord_swc_payment_applicationSearchObj = search.create({
                ...
                filters:
                    [
                        ...
                        // "AND",
                        // ["custrecord_swc_payapply_department","anyof",SWC_CONFIG_DATA.configData().s_department_wltc,SWC_CONFIG_DATA.configData().s_department_wlwc]
                        // // "AND",
            ...
            const customrecord_swc_dzlineSearchObj = search.create({
                ...
                filters:
                    [
                        ...
                        "AND",
                        [
                            ["custrecord_swc_dzline_doc.type","anyof","VendBill","VendCred"],
                            // "AND",
                            // ["custbody_swc_po_fee","anyof","3","6"],
                            "AND",
                            ["custrecord_swc_dzline_doc.custcol_swc_vendor_statement","noneof","@NONE@"],
                            "AND",
                            ["custrecord_swc_dzline_doc.custcol_swc_vendor_application","anyof","@NONE@"],
                        ]
                    ],
                columns:
                    [
