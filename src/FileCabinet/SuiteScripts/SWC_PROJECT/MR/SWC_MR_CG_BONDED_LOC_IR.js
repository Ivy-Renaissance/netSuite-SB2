...
define(['N/search', 'N/record'], function (search, record) {
    ...
    function getNeedData() {
        ...
        var infoSearchObj = search.create({
            ...
            filters:
                [
                    ['custrecord_swc_in_loc_type', 'anyof', '1'],
                    'AND',
                    ['formulanumeric: NVL({custrecord_swc_in_quantity}, 0) - NVL({custrecord_swc_in_allocated_qty}, 0)', 'greaterthan', '0'],
                    'AND',
                    ['isinactive', 'is', 'F'],
                    'AND',
                    ['custrecord_swc_in_sku', 'noneof', '@NONE@'],
                    // 'AND',
                    // ['custrecord_swc_in_number', 'is', '20020214']
                ],
            columns:
                [
                    ...
                    search.createColumn({ name: 'formulanumeric', formula: 'NVL({custrecord_swc_in_quantity}, 0) - NVL({custrecord_swc_in_allocated_qty}, 0)', label: '公式（数值）' }),
                    search.createColumn({ name: 'custrecord_swc_in_quantity', label: '数量' })
                ]
        });
    ...
    function getToId(cg_num) {
        var to_ids = [], search_ids = [], bg_to_ids = [];
        search.create({
            type: 'customrecord_swc_wl_tk_t',
            filters:
                [
                    [
                        ['custrecord_wl_tk_t_bg_t.type', 'anyof', 'TrnfrOrd'],
                        'OR',
                        ['custrecord_wl_tk_t_g_t.type', 'anyof', 'TrnfrOrd']
                    ],
                    'AND',
            ...
            if (result.getValue(result.columns[1]) == 5) {
                var rec_arr = result.getValue(result.columns[2]).split(',');
                if (rec_arr.length > 0) {
                    var srcToId = rec_arr.sort(function (a, b) { return b - a; })[0];
                    bg_to_ids.push(srcToId);
                }
            } else {
        ...
        log.debug('search_ids', search_ids);
        log.debug('bg_to_ids', bg_to_ids);
        if (search_ids.length > 0) {
            to_ids = getSrcToId(search_ids);
        }
        if (bg_to_ids.length > 0) {
            to_ids = to_ids.concat(bg_to_ids);
        }
        return to_ids;
    }
