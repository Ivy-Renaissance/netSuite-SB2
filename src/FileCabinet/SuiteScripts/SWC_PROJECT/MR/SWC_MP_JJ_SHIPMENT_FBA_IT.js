 ...
 *@author ZJG
 *@name SWC_MP_JJ_SHIPMENT_FBA_IT.js
 *@description 亚马逊发货报告处理（从实际发货仓库 转移到 虚拟发出商品仓库）
 */
define(['../common/interface', '../common/moment', 'N/runtime', 'N/record', 'N/search', 'N/format', 'N/error'],
    function (interface, moment, runtime, record, search, format, error) {
        ...
        var shipment_record_type = 'customrecord_swc_jj_fba_shipment'
        function getInputData() {
            var startTime = new Date().getTime();
            log.emergency('getInputData 开始', startTime);
            var acc = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_jj_fs_store' });
            var orderid = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_jj_fs_orderid' });
            var shipdate_start = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_jj_fs_shipdate_start' });
            var shipdate_end = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_jj_fs_shipdate_end' });
            var req_limit = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_jj_fs_limit' });
            var memo = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_jj_fs_memo' });
            ...
            var limit = 3999;
            var filters = [
                { name: 'custrecord_swc_jj_fs_resolved', operator: 'is', values: false },
                { name: 'custrecord_swc_jj_fs_order_type', operator: 'is', values: 'StandardOrder' },
                // { name: 'custrecord_swc_jj_fs_error', operator: 'contains', values: '订单转换成功，待下一次处理' },
            ]
            if (orderid) {
                filters.push({ name: 'custrecord_swc_jj_fs_order_id', operator: 'is', values: orderid })
            };
            if (acc) {
                filters.push({ name: 'custrecord_swc_jj_fs_store', operator: 'anyof', values: acc })
            };
            if (shipdate_end && shipdate_start) {
                filters.push({ name: 'custrecord_swc_jj_fs_shipment_date', operator: 'within', values: [shipdate_start, shipdate_end] })
            }
            if (shipdate_end && !shipdate_start) {
                filters.push({ name: 'custrecord_swc_jj_fs_shipment_date', operator: 'onorbefore', values: shipdate_end })
            }
            if (!shipdate_end && shipdate_start) {
                filters.push({ name: 'custrecord_swc_jj_fs_shipment_date', operator: 'onorafter', values: shipdate_start })
            }
            if (req_limit) {
                limit = req_limit;
            }
            if (memo) {
                filters.push({ name: 'custrecord_swc_jj_fs_error', operator: 'contains', values: memo })
            }
            ...
            search.create({
                ...
                filters: filters,
                columns: [
                    { name: 'custrecord_swc_jj_fs_shipment_date', sort: search.Sort.ASC },
                    { name: 'custrecord_swc_jj_fs_retry', sort: search.Sort.ASC },
                    { name: 'custrecord_swc_jj_fs_quantity_shipped' },
                    { name: 'custrecord_swc_jj_fs_mksu' },
                    { name: 'custrecord_swc_jj_fs_sku' },
                    { name: 'custrecord_swc_jj_fs_order_id' },
                    { name: 'custrecord_swc_jj_fs_store' },
                    { name: 'custrecord_swc_jj_fs_item_price' },
                    { name: 'custrecord_swc_jj_fs_item_tax' },
                    { name: 'custrecord_swc_jj_fs_sales_channel' },
                    { name: 'custrecord_swc_jj_fs_fulfillmentcenterid' },
                    { name: 'custrecord_swc_jj_fs_order_type' },
                    { name: 'custrecord_swc_jj_fs_sp_warehouse_id' },
                    { name: 'custrecord_swc_jj_fs_sp_warehouse_name' },
                ]
            }).run().each(function (rec) {
                orders.push({
                    fsd_id: rec.id,
                    fsd_retry: rec.getValue({ name: 'custrecord_swc_jj_fs_retry', sort: search.Sort.ASC }),
                    fsd_acc_id: rec.getValue('custrecord_swc_jj_fs_store'),
                    fsd_shipment_date: rec.getValue({ name: 'custrecord_swc_jj_fs_shipment_date', sort: search.Sort.ASC }),
                    fsd_sku: rec.getValue('custrecord_swc_jj_fs_sku'),
                    fsd_quantity: rec.getValue('custrecord_swc_jj_fs_quantity_shipped'),
                    fsd_msku: interface.replaceToChinessChar(rec.getValue('custrecord_swc_jj_fs_mksu')),
                    fsd_order_id: rec.getValue('custrecord_swc_jj_fs_order_id'),
                    fsd_fulfillment_center_id: rec.getValue('custrecord_swc_jj_fs_fulfillmentcenterid'),
                    fsd_sales_channel: rec.getValue('custrecord_swc_jj_fs_sales_channel'),
                    fsd_item_price: rec.getValue('custrecord_swc_jj_fs_item_price'),
                    fsd_item_tax: rec.getValue('custrecord_swc_jj_fs_item_tax'),
                    fsd_order_type: rec.getValue('custrecord_swc_jj_fs_order_type'),
                    fsd_sp_warehouse_id: rec.getValue('custrecord_swc_jj_fs_sp_warehouse_id'),
                    fsd_sp_warehouse_name: rec.getValue('custrecord_swc_jj_fs_sp_warehouse_name'),
                })
                return --limit > 0
            })
        ...
        function map(context) {
            ...
            var fsd_msku = obj.fsd_msku;
            var fsd_order_id = obj.fsd_order_id;
            var fsd_sales_channel = obj.fsd_sales_channel;
            var fsd_fulfillment_center_id = obj.fsd_fulfillment_center_id;
            ...
            try {
                ...
                if (!acc_info.shipment_item_location) {
                    throw '请维护店铺发出商品仓库';
                }
                if (!acc_info.plan_metrics) {
                    throw '请维护店铺备货维度';
                }
                var location_info = interface.GetLocationInfo('', '', acc_info.plan_metrics, 'FBA');
                ...
                //2025-12-08  生成 库存转移 单
                var item_info = interface.GetItemInfo('', fsd_sku);
                log.audit('item_info', item_info);

                var it_data = record.create({ type: 'inventorytransfer', isDynamic: true });
                it_data.setValue({ fieldId: 'subsidiary', value: acc_info.subsidiary });
                it_data.setValue({ fieldId: 'location', value: location_info.id });
                it_data.setValue({ fieldId: 'transferlocation', value: acc_info.shipment_item_location });
                it_data.setValue({ fieldId: 'trandate', value: format.parse({ value: fsd_shipment_date, type: 'date' }) });
                it_data.setValue({ fieldId: 'custbody_swc_jj_fba_shipment', value: fsd_id })

                it_data.selectNewLine({ sublistId: 'inventory' });
                it_data.setCurrentSublistValue({ sublistId: 'inventory', fieldId: 'item', value: item_info.id });
                it_data.setCurrentSublistValue({ sublistId: 'inventory', fieldId: 'adjustqtyby', value: fsd_quantity });
                it_data.setCurrentSublistValue({ sublistId: 'inventory', fieldId: 'custcol_swc_msku', value: fsd_msku });
                if (item_info.islotitem || item_info.isserialitem) {
                    IsDynamicInventoryDetails(it_data, 'inventory', fsd_quantity, location_info.id, 'it', acc_info.acc_timezone, item_info.assetaccount)
                } else {
                    SearchInventory(it_data, item_info.id, fsd_quantity, location_info.id, acc_info.acc_timezone, item_info.assetaccount);
                }
                it_data.commitLine({ sublistId: 'inventory' });
                var it_id = it_data.save({ ignoreMandatoryFields: true });
                log.audit('it_id', it_id);
                record.submitFields({
                    type: shipment_record_type,
                    id: fsd_id,
                    values: {
                        custrecord_swc_jj_fs_location: so_ship_location,
                        custrecord_swc_jj_fs_resolved: true,
                        custrecord_swc_jj_fs_error: '',
                        custrecord_swc_jj_fs_relation_it: it_id,
                    },
                    options: {
                ...
                if (fsd_order_type == 'StandardOrder') {
                    ...
                    if (so_id) {
                        ...
                        if (so_obj.ord_status == 'fullyBilled') {
                            record.submitFields({
                                ...
                                values: {
                                    ...
                                    custrecord_swc_jj_fs_error: '没发货已开票',
                                    custrecord_swc_jj_fs_resolved: true,
                                },
                                options: {
                                    ignoreMandatoryFields: true
                        ...
                        search.create({
                            type: 'invoice',
                            filters: [
                                { name: 'custbody_swc_jj_fba_shipment', operator: 'anyof', values: fsd_id },
                        ...
                        if (inv_id.length == 1 && fulfill_id.length == 1) {
                            ...
                            record.submitFields({
                                ...
                                values: {
                                    custrecord_swc_jj_fs_location: so_ship_location,
                                    custrecord_swc_jj_fs_error: '已存在此报告的发票',
                                    custrecord_swc_jj_fs_resolved: true,
                                },
                                options: {
                                    ignoreMandatoryFields: true
                            ...
                            record.submitFields({
                                ...
                                values: {
                                    custrecord_swc_jj_fs_location: so_ship_location,
                                    custrecord_swc_jj_fs_error: '发货有问题，停止发货，需要人工检查问题',
                                },
                                options: {
                                    ignoreMandatoryFields: true
                                }
                        ...
                        if (!flag) {
                            ...
                            record.submitFields({
                                ...
                                values: {
                                    custrecord_swc_jj_fs_location: so_ship_location,
                                    custrecord_swc_jj_fs_error: reason,
                                    custrecord_swc_jj_fs_retry: Number(fsd_retry) + 1
                                },
                                options: {
                                    ignoreMandatoryFields: true
                                }
                        ...
                        if (delete_if) {
                            ...
                            record.submitFields({
                                ...
                                id: fsd_id,
                                values: {
                                    custrecord_swc_jj_fs_error: '重新执行',
                                    custrecord_swc_jj_fs_retry: Number(fsd_retry) + 1
                                },
                                options: {
                                    ignoreMandatoryFields: true
                        ...
                        if (so_obj.subsidiary == location_info.subsidiary) {
                            ...
                            so_itemfulfillment_id = SoToItemFulfillment(fsd_id, so_obj, fsd_order_id, fsd_shipment_date, fsd_msku, fsd_quantity, acc_info, so_ship_location);
                            log.audit('so_itemfulfillment_id', so_itemfulfillment_id);
                            if (so_itemfulfillment_id) {
                                so_invoice_id = SoToInvoice(fsd_id, so_obj, fsd_shipment_date, so_itemfulfillment_id, fsd_msku, fsd_quantity);
                                ...
                                if (so_invoice_id) {
                                    record.submitFields({
                                        ...
                                        values: {
                                            ...
                                            custrecord_swc_jj_fs_resolved: true,
                                            custrecord_swc_jj_fs_error: '',
                                            custrecord_swc_jj_fs_relation_if: so_itemfulfillment_id,
                                            custrecord_swc_jj_fs_relation_iv: so_invoice_id,
                                            custrecord_swc_jj_fs_relation_so: so_obj.so_id,
                                        },
                                        options: {
                                            ignoreMandatoryFields: true
                                        }
        ...
        function SoToItemFulfillment(fsd_id, so_obj, fsd_order_id, fsd_shipment_date, fsd_msku, fsd_quantity, acc_info, so_ship_location) {
            ...
            try {
                ...
                f.setValue({ fieldId: 'custbody_swc_jj_fba_shipment', value: fsd_id }); // 关联发货报告
        ...
        function SoToInvoice(fsd_id, so_obj, fsd_shipment_date, f_id, fsd_msku, fsd_quantity) {
            try {
                fsd_shipment_date = format.parse({ value: fsd_shipment_date, type: 'date' });
                ...
                inv.setValue({ fieldId: 'custbody_swc_jj_fba_shipment', value: fsd_id });
                // 强制限制发票的行等于发货的行
        ...
        function DeleteInfo(so_id) {
            ...
            search.create({
                ...
                columns: [
                    { name: "custbody_swc_jj_fba_shipment" }
                ]
            }).run().each(function (rec) {
                if (rec.getValue('custbody_swc_jj_fba_shipment')) {
                    record.submitFields({
                        type: shipment_record_type,
                        id: rec.getValue('custbody_swc_jj_fba_shipment'),
                        values: {
                            custrecord_swc_jj_fs_resolved: false
                        },
                        options: {
                            enableSourcing: false,
                            ignoreMandatoryFields: true,
