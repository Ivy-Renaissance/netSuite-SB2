 ...
 *@author ZJG
 *@name SWC_MP_JJ_SHIPMENT_OBD.js
 *@description 大货出库单报告处理
 */
define(['../common/interface', '../common/moment', 'N/runtime', 'N/record', 'N/search', 'N/format', 'N/error'],
    function (interface, moment, runtime, record, search, format, error) {
        ...
        var shipment_record_type = 'customrecord_swc_vcpo_ship'
        function getInputData() {
            var startTime = new Date().getTime();
            log.emergency('getInputData 开始', startTime);
            var acc = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_sobd_store' });
            var orderid = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_sobd_orderid' });
            var shipdate_start = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_sobd_shipdate_start' });
            var shipdate_end = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_sobd_shipdate_end' });
            var req_limit = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_sobd_limit' });
            var memo = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_sobd_memo' });
            ...
            var limit = 3999;
            var filters = [
                { name: 'custrecord_swc_vs_resolved', operator: 'is', values: false },
            ]
            if (orderid) {
                filters.push({ name: 'custrecord_swc_vs_sourceorderno', operator: 'is', values: orderid })
            };
            if (acc) {
                filters.push({ name: 'custrecord_swc_vs_linestore', operator: 'anyof', values: acc })
            };
            if (shipdate_end && shipdate_start) {
                filters.push({ name: 'custrecord_swc_vs_out_date', operator: 'within', values: [shipdate_start, shipdate_end] })
            }
            if (shipdate_end && !shipdate_start) {
                filters.push({ name: 'custrecord_swc_vs_out_date', operator: 'onorbefore', values: shipdate_end })
            }
            if (!shipdate_end && shipdate_start) {
                filters.push({ name: 'custrecord_swc_vs_out_date', operator: 'onorafter', values: shipdate_start })
            }
            if (req_limit) {
                limit = req_limit;
            }
            if (memo) {
                filters.push({ name: 'custrecord_swc_vs_error', operator: 'contains', values: memo })
            }
            ...
            search.create({
                ...
                filters: filters,
                columns: [
                    { name: 'custrecord_swc_vs_retry', sort: search.Sort.ASC },
                    { name: 'custrecord_swc_vs_out_date', sort: search.Sort.ASC },
                    { name: 'custrecord_swc_vs_linedeliverynum' },
                    { name: 'custrecord_swc_vs_linealreadyoutnum' },
                    { name: 'custrecord_swc_vs_linemsku' },
                    { name: 'custrecord_swc_vs_sourceorderno' },
                    { name: 'custrecord_swc_vs_linestore' },
                    { name: 'custrecord_swc_vs_ordertype' },
                    { name: 'custrecord_swc_vs_linewarehouseid' },
                    { name: 'custrecord_swc_vs_lineno' },
                    { name: 'custrecord_swc_vs_marketid' },
                    { name: 'custrecord_swc_vs_linesku' },
                ]
            }).run().each(function (rec) {
                orders.push({
                    fsd_id: rec.id,
                    fsd_retry: rec.getValue({ name: 'custrecord_swc_vs_retry', sort: search.Sort.ASC }),
                    fsd_acc_id: rec.getValue('custrecord_swc_vs_linestore'),
                    fsd_shipment_date: rec.getValue({ name: 'custrecord_swc_vs_out_date', sort: search.Sort.ASC }),
                    // fsd_quantity: rec.getValue('custrecord_swc_vs_linedeliverynum'),
                    fsd_quantity: rec.getValue('custrecord_swc_vs_linealreadyoutnum'),
                    fsd_sku: interface.replaceToChinessChar(rec.getValue('custrecord_swc_vs_linemsku')),
                    fsd_order_id: rec.getValue('custrecord_swc_vs_sourceorderno'),
                    fsd_order_type: rec.getValue('custrecord_swc_vs_ordertype'),
                    fsd_sp_warehouse_id: rec.getValue('custrecord_swc_vs_linewarehouseid'),
                    fsd_item_text: rec.getValue('custrecord_swc_vs_linesku'),
                    fsd_marketid: rec.getValue('custrecord_swc_vs_marketid'),
                })
                return --limit > 0
            })
        ...
        function map(context) {
            ...
            var fsd_shippingamount = 0, fsd_others_fee = 0;
            ...
            try {
                if (fsd_order_type == 'vc_po_outbound') {
                    if (!fsd_acc_id) {
                        // throw '不存在店铺';
                    ...
                    if (so_id) {
                        ...
                        if (so_obj.ord_status == 'fullyBilled') {
                            record.submitFields({
                                ...
                                id: fsd_id,
                                values: {
                                    // custrecord_swc_vs_location: location_info.id,
                                    custrecord_swc_vs_error: '没发货已开票',
                                    custrecord_swc_vs_resolved: true,
                                },
                                options: {
                                    ignoreMandatoryFields: true
                                }
                        ...
                        if (!flag) {
                            ...
                            if (!oii_flag) {
                                ...
                                record.submitFields({
                                    ...
                                    id: fsd_id,
                                    values: {
                                        // custrecord_swc_vs_location: so_ship_location,
                                        custrecord_swc_vs_error: reason,
                                        custrecord_swc_vs_retry: Number(fsd_retry) + 1
                                    },
                                    options: {
                                        ignoreMandatoryFields: true
                                    }
                                })
                                interface.SystemLog(externalid, sl_type, '', '5010', reason, false);
                                return
                            }
                        }
                        if (delete_if) {
                        ...
                        if (inv_id.length == 1 && fulfill_id.length == 1) {
                            ...
                            record.submitFields({
                                ...
                                values: {
                                    // custrecord_swc_vs_location: so_ship_location,
                                    custrecord_swc_vs_error: '已存在此报告的发票',
                                    custrecord_swc_vs_resolved: true,
                                },
                                options: {
                                    ignoreMandatoryFields: true
                            ...
                            record.submitFields({
                                ...
                                values: {
                                    custrecord_swc_vs_location: so_ship_location,
                                    custrecord_swc_vs_error: '发货有问题，停止发货，需要人工检查问题',
                                },
                                options: {
                                    ignoreMandatoryFields: true
                                }
                        ...
                        if (so_obj.subsidiary == real_location_info.subsidiary) {
                            ...
                            so_itemfulfillment_id = SoToItemFulfillment(fsd_id, so_obj, fsd_order_id, fsd_shipment_date, fsd_sku, fsd_order_line_no, fsd_quantity, acc_info, so_ship_location, oii_flag, fsd_shippingamount, fsd_others_fee);
                            ...
                            if (so_itemfulfillment_id) {
                                ...
                                if (so_invoice_id) {
                                    record.submitFields({
                                        ...
                                        values: {
                                            custrecord_swc_vs_resolved: true,
                                            custrecord_swc_vs_error: '',
                                            custrecord_swc_vs_relation_if: so_itemfulfillment_id,
                                            custrecord_swc_vs_relation_inv: so_invoice_id,
                                            custrecord_swc_vs_relation_so: so_obj.so_id,
                                        },
                                        options: {
                                            ignoreMandatoryFields: true
                                        }
                ...
                } else if (fsd_order_type == 'vc_collect_outbound') {
                    throw 'VC-COLLECT出库单,暂无处理逻辑';
                } else if (fsd_order_type == 'transfer_outbound') {
                    throw '调拨出库单,暂无处理逻辑';
                } else {
                    throw '未知订单类型,暂不处理';
                }
        ...
        function SoToItemFulfillment(fsd_id, so_obj, fsd_order_id, fsd_shipment_date, fsd_sku, fsd_order_line_no, fsd_quantity, acc_info, so_ship_location, oii_flag, fsd_shippingamount, fsd_others_fee) {
            ...
            search.create({
                ...
                if (rec.getValue('custbody_swc_jj_zfh_shipment')) {
                    record.submitFields({
                        ...
                        values: {
                            custrecord_swc_vs_resolved: false
                        },
                        options: {
                            enableSourcing: false,
                            ignoreMandatoryFields: true,
