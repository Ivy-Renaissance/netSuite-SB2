 ...
 *@author ZJG
 *@name SWC_MP_JJ_TRANSFORM_ORDER.js
 *@description 积加-订单生成（废弃）
 */
define(['N/format', 'N/runtime', 'N/search', 'N/record', 'N/error', '../common/interface', '../common/moment'],
    function (format, runtime, search, record, error, interface, moment) {
        ...
        function getInputData() {
            var data = [];
            try {
                var request_acc = runtime.getCurrentScript().getParameter({ name: 'custscript_jj_to_so_account' });
                var order_id = runtime.getCurrentScript().getParameter({ name: 'custscript_jj_to_so_orderid' });
                var startdate = runtime.getCurrentScript().getParameter({ name: 'custscript_jj_to_so_startdate' });
                var enddate = runtime.getCurrentScript().getParameter({ name: 'custscript_jj_to_so_end_date' });
                if (startdate) {
                ...
                interface.getAccountList().map(function (account) {
                    ...
                    var limit = 200;
                    var filters = [
                        { name: 'custrecord_swc_jj_oc_resolved', operator: 'is', values: false },
                    ]
                    if (order_id) {
                        filters.push({ name: 'custrecord_swc_jj_oc_order_id', operator: 'is', values: order_id })
                    }
                    if (enddate && startdate) {
                        filters.push({ name: 'custrecord_swc_jj_oc_purchase_date', operator: 'within', values: [startdate, enddate] })
                    }
                    if (enddate && !startdate) {
                        filters.push({ name: 'custrecord_swc_jj_oc_purchase_date', operator: 'onorbefore', values: enddate })
                    }
                    if (!enddate && startdate) {
                        filters.push({ name: 'custrecord_swc_jj_oc_purchase_date', operator: 'onorafter', values: startdate })
                    }

                    // 如果先了店铺，则只有等于选择的店铺时才查询，如果没有选择店铺，则都进行查询
                    if ((request_acc && account.id == request_acc) || !request_acc) {
                        filters.push({ name: 'custrecord_swc_jj_oc_store', operator: 'anyof', values: account.id })
                        search.create({
                            type: 'customrecord_swc_jj_order_cache',
                            filters: filters,
                            columns: [
                                { name: 'custrecord_swc_jj_oc_retry', sort: 'ASC' },
                                { name: 'custrecord_swc_jj_oc_purchase_date', sort: 'DESC' },
                                { name: 'custrecord_swc_jj_oc_main_info' },
                            ]
                        }).run().each(function (rec) {
                            data.push({
                                id: rec.id,
                                type: rec.recordType,
                                retry: rec.getValue({ name: 'custrecord_swc_jj_oc_retry', sort: 'ASC' }),
                                order: JSON.parse(rec.getValue('custrecord_swc_jj_oc_main_info')),
                                account: account,
                            });
                            return --limit > 0
                        })
        ...
        function map(context) {
            ...
            try {
                ...
                var local_date_time = format.format({ value: moment.utc(o.purchaseDate).toDate(), type: format.Type.DATETIMETZ, timezone: a.store_time_zone });
                ...
                if (so_id) {
                    ...
                    for (var field_id in interface.fieldsMapping._LIST_ORDERS_.mapping) {
                        if (o[interface.fieldsMapping._LIST_ORDERS_.mapping[field_id]]) {
                            ord.setValue({ fieldId: field_id, value: o[interface.fieldsMapping._LIST_ORDERS_.mapping[field_id]] })
                        }
                    }
                    ...
                    if (o.orderStatus == '4') {
                        ...
                        var line_items = interface.JJGetOrderItems(a, [o.id], '', '', '', 1, 500, []);
                        ...
                        record.submitFields({
                            ...
                            id: obj.id,
                            values: {
                                custrecord_swc_jj_oc_items_info: JSON.stringify(line_items)
                            },
                            options: {
                        ...
                        line_items.map(function (line) {
                            ...
                            for (var i = 0; i < LineCount; i++) {
                                ...
                                if (jj_line_id == line.id) {
                                    ...
                                    ord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'rate', value: line.itemPrice });
                                    for (var field_id in interface.fieldsMapping._LIST_ORDER_ITEMS_.mapping) {
                                        if (line[interface.fieldsMapping._LIST_ORDER_ITEMS_.mapping[field_id]]) {
                                            ord.setCurrentSublistValue({ sublistId: 'item', fieldId: field_id, value: line[interface.fieldsMapping._LIST_ORDER_ITEMS_.mapping[field_id]] });
                                        }
                                    }
                                    break;
                                }
                            ...
                            if (flag) {
                                ...
                                ord.setCurrentSublistText({ sublistId: 'item', fieldId: 'item', text: line.itemId, });
                                ord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'quantity', value: line.quantityOrdered });
                    ...
                    ord.setValue({ fieldId: 'custbody_swc_relation_jj_order_cache', value: obj.id });
                    ...
                    line_items.map(function (line) {
                        ...
                        if (!currency_text) {
                            currency_text = line.currencyCode;
                        }
                        if (!line.itemId) {
                            // 测试中无itemId,给定一个值
                            // line.itemId = 'zjg_test_001'
                ...
                rec.setValue({ fieldId: 'custrecord_swc_jj_oc_error', value: e.message ? e.message : e });
                rec.setValue({ fieldId: 'custrecord_swc_jj_oc_retry', value: Number(obj.retry) + 1 });
        ...
        var mark_resolved = function (type, id, so_id, memo) {
            ...
            rec.setValue({ fieldId: 'custrecord_swc_jj_oc_resolved', value: true });
            if (so_id) {
                rec.setValue({ fieldId: 'custrecord_swc_jj_oc_so', value: so_id });
            }
            if (memo) {
                rec.setValue({ fieldId: 'custrecord_swc_jj_oc_error', value: memo });
            } else {
                rec.setValue({ fieldId: 'custrecord_swc_jj_oc_error', value: '' });
                rec.setValue({ fieldId: 'custrecord_swc_jj_oc_retry', value: '0' });
            }
            rec.save({ ignoreMandatoryFields: true });
        }
