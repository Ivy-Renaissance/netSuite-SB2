...
define(['N/search', 'N/record', '../common/SWC_CONFIG_DATA'], function (search, record, SWC_CONFIG_DATA) {
    ...
    function map(context) {
        try {
            ...
            var previous_to_id = value.to_id, ir_id = value.ir_id, wl_id = value.wl_id;
            //获取贸易条款生成的对应的单据信息
            var tk_info = getTkInfo(previous_to_id);
            log.debug('tk_info', tk_info);
            if (Object.keys(tk_info).length > 0) {
                var po_id = tk_info.po_id, tk_id = tk_info.tk_id, shop_id = tk_info.shop_id, shop_sub_id = tk_info.shop_sub_id, transaction_coefficient = tk_info.transaction_coefficient;
                if (shop_sub_id != CONFIG.SUBSIDIARY_ID_US_OLD && shop_sub_id != 'Fax East Trading LLC') {//只有店铺公司为'Fax East Trading LLC'的才执行后续操作
                    log.debug('error', '店铺子公司不为Fax East Trading LLC，店铺子公司ID：' + shop_sub_id);
                    return;
                }
                //查询发货信息
                ...
                for (var i = 0; i < ir_data.length; i++) {
                    ...
                    soRec.setCurrentSublistValue({ sublistId: 'item', fieldId: 'rate', value: ir_data[i].rate });
                    soRec.setCurrentSublistValue({ sublistId: 'item', fieldId: 'custcol_swc_including_tax_amt', value: ir_data[i].rate });
    ...
    function getTkInfo(previous_to_id) {
        var rec_data = {}, transaction_coefficient = 0;
        search.create({
            ...
            columns:
                [
                    ...
                    'custrecord_wl_tk_t_customer.subsidiarynohierarchy',//店铺子公司
                    'custrecord_wl_tk_t_po',//关联采购订单
                    'custrecord_wl_tk_t_jyll'//交易链路
                ]
        }).run().each(function (result) {
            var t_bg_t = result.getValue(result.columns[0]) || '';
            var shop_id = result.getValue(result.columns[1]) || '';
            var shop_sub_id = result.getValue(result.columns[2]) || '';
            rec_data.tk_id = result.id;
            var po_ids = t_bg_t ? t_bg_t.split(',') : [];
            var po_id;
            if (po_ids.length > 0) {
                po_id = po_ids.sort(function (a, b) { return a - b; })[0];
            } else {
                po_id = result.getValue(result.columns[3]);
                //当已报关单据没有对应单据时（及【成交方式】为海外FOB），获取公司间交易价格系数
                transaction_coefficient = getTransactionCoefficient(result.getValue(result.columns[4]));
            }
            rec_data.po_id = po_id;
            rec_data.shop_id = shop_id;
            rec_data.shop_sub_id = shop_sub_id;
            rec_data.transaction_coefficient = transaction_coefficient;
            return false;
        });
        return rec_data;
    }
