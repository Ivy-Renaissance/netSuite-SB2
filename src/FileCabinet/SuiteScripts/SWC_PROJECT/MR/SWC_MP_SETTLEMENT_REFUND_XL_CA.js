 ...
 *@author ZJG
 *@name SWC_MP_SETTLEMENT_REFUND_XL_CA.js
 *@description 小鹿结算报告（RPA）退款-Shopify平台订单货币与结算货币不一致情况
 */
define(['N/format', 'N/runtime', 'N/search', 'N/record', 'N/error', 'N/config', '../common/interface', '../common/moment', 'N/error'],
    function (format, runtime, search, record, error, config, interface, moment, error) {
        ...
        function getInputData() {
            var acc = runtime.getCurrentScript().getParameter({ name: 'custscript_sett_xl_ca_refund_account' });
            // var marketid = runtime.getCurrentScript().getParameter({ name: 'custscript_sett_xl_refund_marketid' });
            var orderid = runtime.getCurrentScript().getParameter({ name: 'custscript_sett_xl_ca_refund_order_id' });
            var startdate = runtime.getCurrentScript().getParameter({ name: 'custscript_sett_xl_ca_refund_startdate' });
            var enddate = runtime.getCurrentScript().getParameter({ name: 'custscript_sett_xl_ca_refund_enddate' });
            var settlement_id = runtime.getCurrentScript().getParameter({ name: 'custscript_sett_xl_ca_refund_settid' });
            if (startdate) {
            ...
            var filters = [
                ...
                { name: 'custrecord_swc_xl_credit_resolved_ca', operator: 'is', values: false },
            ...
            if (orderid) {
                filters.push({ name: 'custrecord_swc_original_orderid', operator: 'is', values: orderid })
            };
            if (acc) {
        ...
        function map(context) {
            ...
            try {
                ...
                var fee_filters = [
                    ...
                    ['custrecord_swc_ac_type', 'is', 'refund'], 'and',
                    ['custrecord_swc_xl_currency_order', 'is', v.sett_currency], 'and',
                    ['custrecord_swc_xl_credit_resolved_ca', 'is', false]
                ]
                ...
                if (order_id != '- None -') {
                    fee_filters.push('and', ['custrecord_swc_original_orderid', 'is', order_id]);
                } else {
                    fee_filters.push('and', ['custrecord_swc_original_orderid', 'isempty', '']);
                }
        ...
        function reduce(context) {
            ...
            v.map(function (obj) {
                try {
                    ...
                    if (obj.type == 'refund') {
                        rec.setValue({ fieldId: 'custrecord_swc_xl_credit_resolved_ca', value: true });
