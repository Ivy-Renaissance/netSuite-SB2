 ...
 * 贸易条款 V2 - 公司间交易
 */
define(['N/runtime', 'N/record', 'N/search', 'N/log', 'N/currency', 'N/error', '../common/SWC_CONFIG_DATA'], (runtime, record, search, log, currency, error, SWC_CONFIG_DATA) => {
    ...
    const PARAM_PAYLOAD = CONFIG.PARAM_MR_INTERCOMPANY_TRANSACTION_V2_PAYLOAD;
    const LANDED_COST_CATEGORY_BY_FEE_FIELD = CONFIG.LANDED_COST_CATEGORY_BY_FEE_FIELD || {};
    const FOREIGN_SUBSIDIARY_TAX_CODE = CONFIG.TAX_CODE_FOREIGN_SUBSIDIARY_PO;
    ...
    const WL_STEP_FIELD = 'custrecord_swc_wl_mytk_zx_no';
    const VALIDATION_ABORT_NAME = 'INTERCOMPANY_V2_VALIDATION_ABORT';

    // 运抵国 = 美国
    const COUNTRY_US = Number(CONFIG.COUNTRY_ID_US);
    const SUBSIDIARY_US_DEFAULT = Number(CONFIG.SUBSIDIARY_ID_US_DEFAULT);
    const subsidiaryCurrencyCache = {};
    const subsidiaryAreaCache = {};
    ...
    function getInputData() {
        ...
        executionPayload = parsePayload(payloadStr);
        const { wlId, btnType } = executionPayload;
        ...
        try {
            ...
            if (btnType === 'wl') {
                ...
                for (let i = 0; i < lineCount; i++) {
                    ...
                    if (qty > 0 && isBlank(lotNum)) {
                        lotError.push('物流发运明细第' + (i + 1) + '行缺少Lot番号，原PO:' + poId + '，店铺:' + customerId + '，货品:' + skuId + '，SKU:' + sku + '，数量:' + qty);
                    }
                    // 新 PO 子公司：美国固定 77，否则取店铺子公司
                    const newSub = (String(wl_county_lsit) === String(COUNTRY_US)) ? SUBSIDIARY_US_DEFAULT : getCustomerSubsidiary(customerId);
                    ...
                    documentData[key][valueKey]['item_id'] = skuId;
            ...
            if (lotError && lotError.length > 0) {
                log.error('INTERCOMPANY_V2_LOT_MISSING_VALIDATION', {
                    wlId: wlId,
                    btnType: btnType,
                    errorCount: lotError.length,
                    errors: lotError.slice(0, 20)
                });
                writeValidationErrorAndAbort(wlId, '存在物流发运明细缺少Lot番号，已停止生成公司间交易。' + lotError.slice(0, 5).join('；'));
            }

            if (locationError && locationError.length > 0) {
                writeValidationErrorAndAbort(wlId, locationError.join('。'));
            }
            if (Object.keys(documentData).length === 0) {
                writeValidationErrorAndAbort(wlId, '未生成可处理的公司间交易输入数据，请检查前置追踪记录和来源明细');
            }
            return documentData;

        } catch (e) {
            if (e && e.name === VALIDATION_ABORT_NAME) {
                throw e;
            }
            // 失败状态回写
    ...
    function map(context) {
        ...
        // bg 专用段（存在则取）
        const toId = (btnType === 'bg' || btnType === 'qg') ? parts[7] : '';
        const tkID = (btnType === 'bg' || btnType === 'qg') ? parts[8] : '';
        log.debug('Map data', {
        ...
        try {
            if (btnType === 'wl') {
                ...
                if (!noLocationFlag) {
                    ...
                    log.error('intercompany po create pre-line wl', {
                        btnType: 'wl',
                        wlId: String(wlId || ''),
                        poId: String(poId || ''),
                        customerId: String(customerId || ''),
                        entityText: String('IC-' + oldSubsidiaryText || ''),
                        customform: String(newPo.getValue({ fieldId: 'customform' }) || ''),
                        subsidiary: String(newPo.getValue({ fieldId: 'subsidiary' }) || ''),
                        location: String(newPo.getValue({ fieldId: 'location' }) || ''),
                        currency: String(newPo.getValue({ fieldId: 'currency' }) || ''),
                        orderType2: String(newPo.getValue({ fieldId: 'custbody_swc_order_type2' }) || ''),
                        approvalstatus: String(newPo.getValue({ fieldId: 'approvalstatus' }) || '')
                    });
                    ...
                    for (let i = 0; i < srcLineCount; i++) {
                        ...
                        copyLineFields(srcPo, newPo, sublistId, i, COPY_LINE_ID, transaction_coefficient, mapValue[valueKey].lot_qty, true);

                        newPo.commitLine({ sublistId });
                    }
                    newPo.setValue({ fieldId: 'custbody_swc_order_type2', value: 13 });
                    log.error('intercompany po create pre-save wl', {
                        btnType: 'wl',
                        wlId: String(wlId || ''),
                        poId: String(poId || ''),
                        customerId: String(customerId || ''),
                        customform: String(newPo.getValue({ fieldId: 'customform' }) || ''),
                        entity: String(newPo.getValue({ fieldId: 'entity' }) || ''),
                        subsidiary: String(newPo.getValue({ fieldId: 'subsidiary' }) || ''),
                        location: String(newPo.getValue({ fieldId: 'location' }) || ''),
                        currency: String(newPo.getValue({ fieldId: 'currency' }) || ''),
                        orderType2: String(newPo.getValue({ fieldId: 'custbody_swc_order_type2' }) || ''),
                        approvalstatus: String(newPo.getValue({ fieldId: 'approvalstatus' }) || ''),
                        itemLineCount: String(newPo.getLineCount({ sublistId: 'item' }) || 0)
                    });
                    ...
                    for (let i = 0; i < srcLineCount; i++) {
                        ...
                        copyLineFields(srcPo, soRec, sublistId, i, COPY_LINE_ID, transaction_coefficient, mapValue[valueKey].lot_qty, false);
                ...
                // 新 PO 子公司：美国固定 77，否则取店铺子公司
                var newSub = (String(wl_county_lsit) === String(COUNTRY_US)) ? SUBSIDIARY_US_DEFAULT : customerSub;
                ...
                const currencyId = transaction_coefficient.transaction_currency || srcPo.getValue('currency');
                newPo.setValue({ fieldId: 'currency', value: currencyId });
                log.error('intercompany po create pre-line bgqg', {
                ...
                newPo.setValue({ fieldId: 'approvalstatus', value: 2 });
                newPo.setValue({ fieldId: 'custbody_swc_order_type2', value: 13 });
                log.error('intercompany po create pre-save bgqg', {
                    btnType: String(btnType || ''),
                    wlId: String(wlId || ''),
                    poId: String(poId || ''),
                    customerId: String(customerId || ''),
                ...
                srcPo.save();
                log.debug('公司间交易 PO（bg/qg）做成', newPoId);
                // 公司间交易 SO
                const soRec = record.create({
                    type: record.Type.SALES_ORDER,
                    isDynamic: true,
                ...
                log.debug('公司间交易 SO（bg/qg）做成', soId);
    ...
    function reduce(context) {
        ...
        const btnType = ids[7];
        const tkId = (btnType === 'bg' || btnType === 'qg') ? ids[8] : '';
        ...
        try {
            if (noLocationFlag == false) {
                ...
                for (let i = 0; i < lineCount; i++) {
                    ...
                    if (!isBlank(lotNum)) {
                        ...
                        try {
                            ...
                            throw error.create({
                                name: 'LOT_ISSUE_NUMBER_INVALID',
                                message: 'Lot番号在出库库存明细中不可用，已停止公司间交易后续处理。SO:' + soId
                                    + '，行:' + (i + 1)
                                    + '，货品:' + String(ifRec.getCurrentSublistValue({ sublistId, fieldId: 'item' }) || '')
                                    + '，地点:' + String(so_loc || '')
                                    + '，Lot番号:' + String(lotNum || '')
                                    + '，原始错误:' + (lotError.message || lotError.name || lotError),
                                notifyOff: true
                            });
                        }
                ...
                const irSubsidiaryId = irRec.getValue({ fieldId: 'subsidiary' }) || '';
                const landedCostCurrencyId = getSubsidiaryCurrency(irSubsidiaryId) || irCurrencyId || '';
                ...
                for (let i = 0; i < irLineCount; i++) {
                    ...
                    if (!isBlank(lotNum)) {
                        ...
                        inventorydetail.selectNewLine({ sublistId: 'inventoryassignment' });
                        log.debug('lot assignment before receiptinventorynumber', {
                            单据类型: 'Item Receipt',
                            来源单据: String(newPoId || ''),
                            货品: String(item_id || ''),
                            行号: String(lineNo || ''),
                ...
                if (btnType === 'bg' || btnType === 'wl' || (btnType === 'qg' && terms_of_trade == '4')) {
                    ...
                    for (let i = 0; i < lineCount; i++) {
                        ...
                        if (oldPoId == poId) {
                            ...
                            var trailer_fee_currency = wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_trailer_fee_c', line: i }) || '';
                            var cda_fee = toNumber(wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_cda_fee', line: i })) || 0;
                            var cda_fee_currency = wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_cda_fee_c', line: i }) || '';
                            var em_ffc = toNumber(wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_ffc', line: i })) || 0;
                            var em_ffc_currency = wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_ffc_c', line: i }) || '';
                            ...
                            var hyf_fee_currency = wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_hyf_fee_c', line: i }) || '';
                            var qgf_fee = toNumber(wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_qgf_fee', line: i })) || 0;
                            var qgf_fee_currency = wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_qgf_fee_c', line: i }) || '';
                            var jkgs_fee = toNumber(wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_jkgs_fee', line: i })) || 0;
                            var jkgs_fee_currency = wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_jkgs_fee_c', line: i }) || '';
                            var hdf_fee = toNumber(wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_hdf_fee', line: i })) || 0;
                            var hdf_fee_currency = wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_hdf_fee_c', line: i }) || '';
                            var tcf_fee = toNumber(wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_tcf_fee', line: i })) || 0;
                            var tcf_fee_currency = wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_tcf_fee_c', line: i }) || '';

                            var rkcz = toNumber(wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_rkcz_fee', line: i })) || 0;
                            var rkcz_currency = wlRec.getSublistValue({ sublistId: WL_DETAIL_SUBLIST, fieldId: 'custrecord_swc_wl_d_em_rkcz_fee_c', line: i }) || '';

                            trailer_fee = convertLandedCostAmountBySubsidiaryCurrency(trailer_fee, trailer_fee_currency, landedCostCurrencyId, irTranDate);
                            cda_fee = convertLandedCostAmountBySubsidiaryCurrency(cda_fee, cda_fee_currency, landedCostCurrencyId, irTranDate);
                            em_ffc = convertLandedCostAmountBySubsidiaryCurrency(em_ffc, em_ffc_currency, landedCostCurrencyId, irTranDate);
                            bxf_fee = convertLandedCostAmountBySubsidiaryCurrency(bxf_fee, bxf_fee_currency, landedCostCurrencyId, irTranDate);
                            hyf_fee = convertLandedCostAmountBySubsidiaryCurrency(hyf_fee, hyf_fee_currency, landedCostCurrencyId, irTranDate);
                            qgf_fee = convertLandedCostAmountBySubsidiaryCurrency(qgf_fee, qgf_fee_currency, landedCostCurrencyId, irTranDate);
                            jkgs_fee = convertLandedCostAmountBySubsidiaryCurrency(jkgs_fee, jkgs_fee_currency, landedCostCurrencyId, irTranDate);
                            hdf_fee = convertLandedCostAmountBySubsidiaryCurrency(hdf_fee, hdf_fee_currency, landedCostCurrencyId, irTranDate);
                            tcf_fee = convertLandedCostAmountBySubsidiaryCurrency(tcf_fee, tcf_fee_currency, landedCostCurrencyId, irTranDate);
                            rkcz = convertLandedCostAmountBySubsidiaryCurrency(rkcz, rkcz_currency, landedCostCurrencyId, irTranDate);


                            addLandedCostFeeAmount(poIds, key, {
                            ...
                            var compatibleKey = buildIntercompanyReceiptCompatibleLandedCostKey(itemId, d_sku, d_country, d_location_type, d_region, yl, d_customer);
                            if (compatibleKey && compatibleKey !== key) {
                                addLandedCostFeeAmount(poIds, compatibleKey, {
                                    trailer_fee: trailer_fee,
                                    cda_fee: cda_fee,
                                    em_ffc: em_ffc,
                                    bxf_fee: bxf_fee,
                    ...
                    for (var landedKey in poIds) {
                        if (!Object.prototype.hasOwnProperty.call(poIds, landedKey)) continue;
                        log.debug('landed cost source line', {
                            匹配key: landedKey,
                            货品: String(landedKey.split('_')[0] || ''),
                            原始SKU: String(landedKey.split('_')[1] || ''),
                            国家: String(landedKey.split('_')[2] || ''),
                            仓库类型: String(landedKey.split('_')[3] || ''),
                            区域: String(landedKey.split('_')[4] || ''),
                            品级: String(landedKey.split('_')[5] || ''),
                            店铺: String(landedKey.split('_')[6] || ''),
                            费用: poIds[landedKey]
                        });
                    }
                    log.debug('terms_of_trade', terms_of_trade);
            ...
            if (btnType === 'wl') {
                ...
                const tradeLinkId = resolveTradeLinkIdByShop(customerId);
                ...
                if (!isBlank(tradeLinkId)) {
                    wl_tk_t.setValue({ fieldId: 'custrecord_wl_tk_t_jyll', value: tradeLinkId });
                }
                ...
                if (btnType === 'qg') {
                    ...
                    const sourceLocationAttribute = resolveLocationAttribute(btnType, terms_of_trade);
                    const sourceLocationId = String(to_start_loc || findLocationInternalId(customerId, sourceLocationAttribute, '', to_sub, warehouse_code) || '');
                    const locationAttribute1 = resolveLocationAttribute(btnType, 'TO');
                    let toLocationId = findLocationInternalId(customerId, locationAttribute1, '', to_sub, warehouse_code);
                    if (!toLocationId) {
                        toLocationId = findLocationInternalId(customerId, '6', '', to_sub, warehouse_code);
                    }
                    if (!sourceLocationId || !toLocationId) {
                        writeValidationErrorAndAbort(
                            wlId,
                            '公司间交易清关后续转移单地点缺失。来源地点:' + (sourceLocationId || '空')
                            + '，目标地点:' + (toLocationId || '空')
                            + '，店铺:' + customerId
                            + '，子公司:' + to_sub
                            + '，仓库代码:' + warehouse_code
                    ...
                    newToRec.setValue({ fieldId: 'location', value: sourceLocationId });
                    ...
                    for (var i = 0; i < new_to_item_arr_hb.length; i++) {
                        ...
                        if (new_to_item_arr_hb[i].lot) {
                            ...
                            for (var j = 0; j < new_to_item_arr_hb[i].lot.length; j++) {
                                inventorydetail.selectNewLine({ sublistId: 'inventoryassignment' });
                                log.debug('lot assignment before qg transfer receiptinventorynumber', {
                                    单据类型: 'Transfer Order',
                                    来源单据: String(newToRecId || ''),
                                    货品: String(new_to_item_arr_hb[i].item_id || ''),
                                    数量: Number(new_to_item_arr_hb[i].lot[j].quantity || 0),
                                    Lot番号: String(new_to_item_arr_hb[i].lot[j].lotNumber || '')
                                });
                                inventorydetail.setCurrentSublistText({
                                    sublistId: 'inventoryassignment',
                                    fieldId: 'receiptinventorynumber',
                    ...
                    record.submitFields({
                        ...
                        values: (function () {
                            ...
                            if (!isNaN(payload.currentStep)) {
                                values[WL_STEP_FIELD] = String(payload.currentStep + 1);
                            }
                            return values;
                        })()
                    });
                    log.error('intercompany v2 summarize status updated', {
                        wlId: String(payload.wlId || ''),
                        tkStatus: String(statusValue || ''),
                        nextStep: isNaN(payload.currentStep) ? '' : String(payload.currentStep + 1)
                    });
                } else {
                    log.error('intercompany v2 summarize status skipped', {
                        wlId: String(payload.wlId || ''),
                        currentStep: String(payload.currentStep || ''),
                        buttonId: String(payload.buttonId || ''),
                        buttonText: String(payload.buttonText || ''),
    ...
    function getTransactionCoefficient(po_sub, customerSub, tranDate) {
        ...
        var exchangeRate = 1;
        var filters_arr = [];
        if (Number(customerSub) === SUBSIDIARY_US_DEFAULT) {//公司为Lettoi LLC指定查询中转公司
    ...
    function isForeignSubsidiary(subsidiaryId) {
        var key = String(subsidiaryId || '');
        if (!key) return false;
        if (Object.prototype.hasOwnProperty.call(subsidiaryAreaCache, key)) {
            return subsidiaryAreaCache[key] === String(CONFIG.s_subway_gw || '');
        }
        ...
        var areaArr = subsidiaryInfo.custrecord_swc_subway_001_ || [];
        var areaValue = (areaArr[0] && areaArr[0].value) ? String(areaArr[0].value) : '';
        subsidiaryAreaCache[key] = areaValue;
        return areaValue === String(CONFIG.s_subway_gw || '');
    }
    ...
    function parsePayload(payloadStr) {
        var payload = {};
        try {
            payload = JSON.parse(String(payloadStr || '{}'));
        } catch (e) {
            payload = {};
        }

        var buttonText = String(payload.buttonText || payload.buttonLabel || '');
        var routeBranch = String(payload.routeBranch || '');
        return {
            raw: payload,
            recId: String(payload.recId || ''),
            wlId: String(payload.recId || ''),
            currentStep: parseInt(String(payload.currentStep || ''), 10),
            routeBranch: routeBranch,
            tradeLinkId: String(payload.configId || ''),
            tradeLinkName: String(payload.configName || ''),
            ...
            currentStepRows: Array.isArray(payload.currentStepRows) ? payload.currentStepRows : [],
            btnType: resolveBtnTypeForIntercompany(payload.buttonId, buttonText, routeBranch)
        };
    }

    function resolveTradeLinkIdByShop(customerId) {
        var stepRows = executionPayload.currentStepRows || [];
        var shopSubId = String(getCustomerSubsidiary(customerId) || '');
        if (shopSubId) {
            for (var i = 0; i < stepRows.length; i++) {
                var row = stepRows[i] || {};
                if (String(row.shopSubId || '') === shopSubId && !isBlank(row.configId)) {
                    return String(row.configId || '');
                }
            }
        }
        return String(executionPayload.tradeLinkId || '');
    }

    function resolveBtnTypeForIntercompany(buttonId, buttonText, routeBranch) {
        var id = String(buttonId || '');
        if (id === '1') return 'wl';
        if (id === '2') return 'bg';
        if (id === '3') return 'qg';
        var text = String(buttonText || '');
        if (text.indexOf('供应商已出货') !== -1) return 'wl';
        if (text.indexOf('已报关') !== -1) return 'bg';
        if (text.indexOf('清关') !== -1) return 'qg';
        return '';
    }
    ...
    function buildIntercompanyReceiptCompatibleLandedCostKey(itemId, originSku, country, locationType, region, grade, customer) {
        return buildLandedCostKey(originSku, originSku, country, locationType, region, grade, customer);
    }

    function addLandedCostFeeAmount(targetMap, key, feeObj) {
        if (!targetMap || !key || !feeObj) return;
        ...
        for (var feeKey in feeObj) {
            if (!Object.prototype.hasOwnProperty.call(feeObj, feeKey)) continue;
            targetMap[key][feeKey] = Number(targetMap[key][feeKey] || 0) + Number(feeObj[feeKey] || 0);
        }
    }

    function convertLandedCostAmountBySubsidiaryCurrency(amount, sourceCurrencyId, targetCurrencyId, tranDate) {
        amount = Number(amount || 0);
        if (!amount) return 0;
        if (!sourceCurrencyId || !targetCurrencyId || String(sourceCurrencyId) === String(targetCurrencyId)) {
            return amount;
        }
        return roundAmount(amount * getStandardExchangeRate(sourceCurrencyId, targetCurrencyId, tranDate));
    }
    ...
    function resolveLocationAttribute(btnType, terms_of_trade) {
        var warehouseAttribute = String(executionPayload.warehouseAttribute || '');
        var startingWarehouseAttribute = String(executionPayload.startingWarehouseAttribute || '');
        if (String(terms_of_trade || '') === 'TO' && startingWarehouseAttribute) {
            return startingWarehouseAttribute;
        }
     ...
     */
    function copyLineFields(srcRec, dstRec, sublistId, srcLine, fieldIds, transactionInfo, qty, applyForeignTaxCode) {
        ...
        for (let i = 0; i < fieldIds.length; i++) {
            ...
            if (fid == 'rate') {
                ...
                const tax_amt = srcRec.getSublistValue({ sublistId, fieldId: 'custcol_swc_including_tax_amt', line: srcLine });
                log.debug('tax_amt_for_amount', tax_amt);
                v = tax_amt * exchangeRate * (1 + additiveMultiplier) * qty;
        ...
        if (applyForeignTaxCode) {
            var dstSubsidiary = dstRec.getValue({ fieldId: 'subsidiary' }) || '';
            if (isForeignSubsidiary(dstSubsidiary)) {
                dstRec.setCurrentSublistValue({ sublistId, fieldId: 'taxcode', value: FOREIGN_SUBSIDIARY_TAX_CODE });
                log.error('intercompany po line foreign taxcode forced', {
                    subsidiary: String(dstSubsidiary || ''),
                    taxcode: String(FOREIGN_SUBSIDIARY_TAX_CODE),
                    srcLine: String(srcLine || ''),
                    item: String(dstRec.getCurrentSublistValue({ sublistId: sublistId, fieldId: 'item' }) || '')
                });
            }
        }
    }
    ...
    function buildReduceKey(p) {
        ...
        if (p.btnType === 'bg' || p.btnType === 'wl' || p.btnType === 'qg') {
    ...
    function applyLandedCostPerLine(irRec, poIds, terms_of_trade) {
        try {
            const feeMaping = LANDED_COST_CATEGORY_BY_FEE_FIELD;
            ...
            for (let i = 0; i < lineCount; i++) {
                ...
                log.debug('landed cost ir line', {
                    行: i,
                    匹配key: key,
                    货品: String(itemId || ''),
                    原始SKU: String(originSku || ''),
                    国家: String(d_country || ''),
                    仓库类型: String(d_location_type || ''),
                    区域: String(d_region || ''),
                    品级: String(yl || ''),
                    店铺: String(d_customer || ''),
                    是否命中来源: Object.prototype.hasOwnProperty.call(poIds || {}, key) ? '是' : '否'
                });
    ...
    function writeValidationErrorAndAbort(wlId, message) {
        ...
        if (wlId) {
            record.submitFields({
                ...
                id: wlId,
                values: {
                    custrecord_my_tk_error_message: safeMessage
                }
            });
        }
        throw error.create({
            name: VALIDATION_ABORT_NAME,
            message: safeMessage || '贸易条款校验失败',
            notifyOff: true
        });
    }
