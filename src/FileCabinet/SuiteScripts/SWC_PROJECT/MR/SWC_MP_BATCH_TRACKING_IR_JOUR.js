 ...
 *@author ZJG
 *@name SWC_MP_BATCH_TRACKING_IR_JOUR.js
 *@description 成本还原-批次跟踪 退货入库单生成日记账
 */
define(['../common/interface', '../common/moment', 'N/runtime', 'N/record', 'N/search', 'N/format', 'N/error'],
    function (interface, moment, runtime, record, search, format, error) {
        ...
        function getInputData() {
            ...
            var ir_id = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_btj_ir_id' });
            var location_id = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_btj_ir_location_id' });
            var start_date = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_btj_ir_start_date' });
            var end_date = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_btj_ir_end_date' });
            ...
            var filters = [
                ...
                { name: 'custrecord_swc_bt_type', operator: 'anyof', values: ['5'] },
                { name: 'custrecord_swc_bt_type_id', operator: 'noneof', values: ['@NONE@'] },
            ]
            if (ir_id) {
                filters.push({ name: 'custrecord_swc_bt_type_id', operator: 'anyof', values: ir_id })
            }
            if (location_id) {
            ...
            search.create({
                ...
                data.push({
                    ir_id: rec.getValue(rec.columns[0])
                });
                return --limit > 0
            })
            log.audit('data', data);
        ...
        function map(context) {
            try {
                ...
                var fee_filters = [
                    ...
                    { name: 'custrecord_swc_bt_resolved', operator: 'is', values: false },
                    { name: 'custrecord_swc_bt_type_id', operator: 'anyof', values: ir_id },
                ]
                ...
                var ir_rec = record.load({ type: 'itemreceipt', id: ir_id });
                var ordertype = ir_rec.getValue('ordertype');
                if (ordertype == 'TrnfrOrd') {
                    for (var i = 0; i < cacheids.length; i++) {
                        context.write({
                            key: ir_id + '.' + cacheids[i],
                            value: {
                                'cache_id': cacheids[i],
                                'ir_id': ir_id,
                                'type': 'no_deal',
                ...
                search.create({
                    ...
                    filters: [
                        { name: 'internalId', operator: 'is', values: ir_id },
                ...
                // var difFee = [];
                // for (let i = 0; i < ir_data.length; i++) {
                //     for (let j = 0; j < FeeTypeArray.length; j++) {
                //         if (ir_data[i].item_text == FeeTypeArray[j].sku_text && ir_data[i].serinalnumber == FeeTypeArray[j].batch_text) {
                //             if (difFee.length) {
                //                 var flag = true;
                ...
                //                         difFee[k].dif_fee = interface.accSub(Math.abs(difFee[k].dif_fee), Math.abs(interface.accMul(FeeTypeArray[j].fee, FeeTypeArray[j].qty)));
                //                         flag = false;
                //                         break
                //                     }
                //                 }
                //                 if (flag) {
                //                     difFee.push({
                //                         item_text: ir_data[i].item_text,
                //                         serinalnumber: ir_data[i].serinalnumber,
                //                         dif_fee: interface.accSub(Math.abs(ir_data[i].amount), Math.abs(interface.accMul(FeeTypeArray[j].fee, FeeTypeArray[j].qty)))
                //                     });
                //                 }
                //             } else {
                //                 difFee.push({
                ...
                if (FeeTypeArray.length) {
                    ...
                    jour_fy.setValue({ fieldId: 'memo', value: '成本还原 退货入库日记账' });
                    jour_fy.setValue({ fieldId: 'approvalstatus', value: 2 });
                    jour_fy.setValue({ fieldId: 'custbody_swc_bill_create_receipt', value: ir_id });
                    ...
                    if (runtime.accountId == '11297254_SB1') {
                        var ir_account = '1417';//T6601.14.01.01 销售费用-销货成本_采购成本_采购价格
                        var dif_account = '1523';//T6601.99	核算差异
                    } else {
                        var ir_account = '959';//T6601.14.01 销售费用-销货成本_采购成本_采购价格
                        var dif_account = '981';//T6601.99	核算差异
                    }
                    for (let i = 0; i < ir_data.length; i++) {
                        jour_fy.selectNewLine({ sublistId: 'line' })
                        jour_fy.setCurrentSublistValue({ sublistId: 'line', fieldId: 'account', value: ir_account });
                        jour_fy.setCurrentSublistValue({ sublistId: 'line', fieldId: 'memo', value: ir_data[i].item_text + '  ' + ir_data[i].serinalnumber });
                        jour_fy.setCurrentSublistValue({ sublistId: 'line', fieldId: "debit", value: ir_data[i].amount }); //借
                        jour_fy.commitLine({ sublistId: 'line' })
                    }
