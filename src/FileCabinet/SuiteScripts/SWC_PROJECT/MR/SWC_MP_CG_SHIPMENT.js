 ...
 *@author ZJG
 *@name SWC_MP_CG_SHIPMENT.js
 *@description CG库存分类账-发货、调整
 */
define(['../common/interface', '../common/moment', 'N/runtime', 'N/record', 'N/search', 'N/format', 'N/error'],
    function (interface, moment, runtime, record, search, format, error) {
        ...
        var shipment_record_type = 'customrecord_swc_cg_inventoryledger'
        function getInputData() {
            var startTime = new Date().getTime();
            log.emergency('getInputData 开始', startTime);
            var acc = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_cg_store' });
            var orderid = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_cg_orderid' });
            var shipdate_start = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_cg_shipdate_start' });
            var shipdate_end = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_cg_shipdate_end' });
            var req_limit = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_cg_limit' });
            var memo = runtime.getCurrentScript().getParameter({ name: 'custscript_swc_cg_memo' });
            ...
            var limit = 200;
            var filters = [
                ['custrecord_swc_cg_resolved', 'is', false], 'and',
                // [
                // ['custrecord_swc_cg_eventtype', 'is', 'Customer Sale - Multi Channel'], 'or',
                ['custrecord_swc_cg_eventtype', 'is', 'Customer Sale - Wayfair']
                // ]
            ]
            if (orderid) {
                filters.push('and', ['custrecord_swc_cg_transactionnumber', 'is', orderid]);
            };
            if (shipdate_end && shipdate_start) {
                filters.push('and', ['custrecord_swc_cg_date', 'within', [shipdate_start, shipdate_end]]);
            }
            if (shipdate_end && !shipdate_start) {
                filters.push('and', ['custrecord_swc_cg_date', 'onorbefore', [shipdate_end]]);
            }
            if (!shipdate_end && shipdate_start) {
                filters.push('and', ['custrecord_swc_cg_date', 'onorafter', [shipdate_start]]);
            }
            if (memo) {
                filters.push('and', ['custrecord_swc_cg_error', 'contains', memo]);
            }
            if (req_limit) {
                limit = req_limit;
            }
            ...
            search.create({
                ...
                columns: [
                    ...
                    { name: 'custrecord_swc_cg_facility' },
                    { name: 'custrecord_swc_cg_item' },
                    { name: 'custrecord_swc_cg_location' },
                    { name: 'custrecord_swc_cg_eventtype' },
                ]
            }).run().each(function (rec) {
                orders.push({
                    fsd_id: rec.id,
                    fsd_retry: rec.getValue({ name: 'custrecord_swc_cg_retry', sort: search.Sort.ASC }),
                    fsd_shipment_date: rec.getValue({ name: 'custrecord_swc_cg_date', sort: search.Sort.ASC }),
                    fsd_quantity: rec.getValue('custrecord_swc_cg_quantity'),
                    fsd_sku: interface.replaceToChinessChar(rec.getValue('custrecord_swc_cg_partnumber')),
                    fsd_facility: rec.getValue('custrecord_swc_cg_facility'),
                    fsd_order_id: rec.getValue('custrecord_swc_cg_transactionnumber'),
                    fsd_item: rec.getValue('custrecord_swc_cg_item'),
                    fsd_location: rec.getValue('custrecord_swc_cg_location'),
                    fsd_eventtype: rec.getValue('custrecord_swc_cg_eventtype'),
                });
                return --limit > 0
            })
        ...
        function map(context) {
            ...
            var fsd_eventtype = obj.fsd_eventtype;
            ...
            try {
                ...
                if (so_id) {
                    ...
                    if (so_obj.ord_status == 'fullyBilled') {
                        record.submitFields({
                            ...
                            id: fsd_id,
                            values: {
                                custrecord_swc_cg_error: '没发货已开票',
                    ...
                    search.create({
                        type: 'invoice',
                        filters: [
                            { name: 'custbody_swc_gl_cg_inventoryledger', operator: 'anyof', values: fsd_id },
                    ...
                    if (inv_id.length == 1 && fulfill_id.length == 1) {
                        ...
                        record.submitFields({
                            ...
                            id: fsd_id,
                            values: {
                                custrecord_swc_cg_error: '已存在此报告的发票',
                                custrecord_swc_cg_resolved: true,
                                custrecord_swc_cg_itemfulfillment: fulfill_id[0],
                                custrecord_swc_cg_invoice: inv_id[0],
                                custrecord_swc_cg_salesorder: so_obj.so_id,
                            },
                            options: {
                                ignoreMandatoryFields: true
                            }
                        ...
                        record.submitFields({
                            ...
                            id: fsd_id,
                            values: {
                                custrecord_swc_cg_error: '发货有问题，停止发货，需要人工检查问题',
                            },
                            options: {
                                ignoreMandatoryFields: true
                            }
                    ...
                    var location_info = interface.GetLocationInfo(fsd_sp_warehouse_id, '', acc_info.plan_metrics, 'CG', location_attribute);
                    log.audit('location_info', location_info);
                    ...
                    if (item_info.item_cplb == '7') {
                        var pj_location_info = interface.GetLocationInfo(fsd_sp_warehouse_id, '', acc_info.pj_plan_metrics, 'CG', location_attribute);
                            ...
                            if (so_invoice_id) {
                                record.submitFields({
                                    ...
                                    values: {
                                        ...
                                        custrecord_swc_cg_itemfulfillment: so_itemfulfillment_id,
                                        custrecord_swc_cg_invoice: so_invoice_id,
                                        custrecord_swc_cg_salesorder: so_obj.so_id,
                                    },
                                    options: {
                                        ignoreMandatoryFields: true
                ...
                record.submitFields({
                    ...
                    values: {
                        ...
                        custrecord_swc_cg_retry: Number(fsd_retry) + 1
                    },
                    options: {
                        ignoreMandatoryFields: true
                    }
        ...
        function SoToItemFulfillment(fsd_id, so_obj, fsd_order_id, fsd_shipment_date, fsd_quantity, acc_info, so_ship_location, fsd_item_info, fsd_sku) {
            ...
            try {
                ...
                f.setValue({ fieldId: 'custbody_swc_gl_cg_inventoryledger', value: fsd_id }); // 关联发货报告
        ...
        function SoToInvoice(fsd_id, so_obj, fsd_shipment_date, f_id, fsd_quantity, fsd_item_info, fsd_sku) {
            try {
                ...
                inv.setValue({ fieldId: 'custbody_swc_gl_cg_inventoryledger', value: fsd_id });
                // 强制限制发票的行等于发货的行
                ...
                for (var i = seted; i < len; i++) {
                    ...
                    if (itds == fsd_item_info.item_id && msku == fsd_sku && fsd_quantity <= qty && ck) {//(&& oii_flag ? true : fsd_order_line_no == platform_line_id) 
                        seted = i + 1
                        ck = false
        ...
        function DeleteInfo(so_id) {
            ...
            search.create({
                ...
                columns: [
                    { name: "custbody_swc_gl_cg_inventoryledger" }
                ]
            }).run().each(function (rec) {
                if (rec.getValue('custbody_swc_gl_cg_inventoryledger')) {
                    record.submitFields({
                        type: shipment_record_type,
                        id: rec.getValue('custbody_swc_gl_cg_inventoryledger'),
                        values: {
