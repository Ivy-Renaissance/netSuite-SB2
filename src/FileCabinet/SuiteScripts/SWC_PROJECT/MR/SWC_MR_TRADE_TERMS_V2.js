 ...
 * 贸易条款 V2 - 转移单
 */
define(['N/runtime', 'N/record', 'N/search', 'N/log', 'N/error', 'N/currency', '../common/SWC_CONFIG_DATA'], (runtime, record, search, log, error, currency, SWC_CONFIG_DATA) => {
    const CONFIG = SWC_CONFIG_DATA.configData();

    // -----------------------------
    // 常量：脚本参数
    // -----------------------------
    const PARAM_PAYLOAD = CONFIG.PARAM_MR_TRADE_TERMS_V2_PAYLOAD;
    ...
    const VALIDATION_ABORT_NAME = 'TRADE_TERMS_V2_VALIDATION_ABORT';
    ...
    function getInputData() {
        try {
            ...
            log.error('trade terms v2 mr raw payload', String(payloadStr || ''));
            executionPayload = parseV2Payload(payloadStr);
            const wlId = executionPayload.wlId;
            const btnType = executionPayload.btnType;
            ...
            if (btnType === 'wl') {
                ...
                for (let i = 0; i < lineCount; i++) {
                    ...
                    const trailer_fee_currency = wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_trailer_fee_c', line: i }) || '';
                    const cda_fee = toNumber(wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_cda_fee', line: i })) || 0;
                    const cda_fee_currency = wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_cda_fee_c', line: i }) || '';
                    const em_ffc = toNumber(wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_ffc', line: i })) || 0;
                    const em_ffc_currency = wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_ffc_c', line: i }) || '';
                    const bxf_fee = toNumber(wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_bxf_fee', line: i })) || 0;
                    const bxf_fee_currency = wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_bxf_fee_c', line: i }) || '';
                    const hyf_fee = toNumber(wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_hyf_fee', line: i })) || 0;
                    const hyf_fee_currency = wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_hyf_fee_c', line: i }) || '';
                    const qgf_fee = toNumber(wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_qgf_fee', line: i })) || 0;
                    const qgf_fee_currency = wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_qgf_fee_c', line: i }) || '';
                    const jkgs_fee = toNumber(wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_jkgs_fee', line: i })) || 0;
                    const jkgs_fee_currency = wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_jkgs_fee_c', line: i }) || '';
                    const hdf_fee = toNumber(wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_hdf_fee', line: i })) || 0;
                    const hdf_fee_currency = wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_hdf_fee_c', line: i }) || '';
                    const tcf_fee = toNumber(wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_tcf_fee', line: i })) || 0;
                    const tcf_fee_currency = wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_tcf_fee_c', line: i }) || '';

                    const rkcz = toNumber(wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_rkcz_fee', line: i })) || 0;
                    const rkcz_currency = wlRec.getSublistValue({ sublistId: feeSubID, fieldId: 'custrecord_swc_wl_d_em_rkcz_fee_c', line: i }) || '';
            ...
            if (btnType === 'bg') {
                ...
                if (Number(terms_of_trade) === 1) {
                    ...
                    if (tkRows.length === 0) {
                        writeValidationErrorAndAbort(wlId, '未找到公司间交易明细（贸易条款追踪记录为空或字段缺失）');
                    }
                ...
                if (tkCount === 0) {
                    writeValidationErrorAndAbort(wlId, '未找到报关前置转移单追踪记录，请先确认上一流程是否已完成');
                }
                ...
                if (list.length === 0) {
                    writeValidationErrorAndAbort(wlId, '未生成可处理的报关输入数据，请检查贸易条款追踪记录和来源转移单明细');
                }
                return list;
            }
            ...
            if (btnType === 'qg') {
                ...
                if (Number(terms_of_trade) === 2) {
                    ...
                    if (tkRows.length === 0) {
                        writeValidationErrorAndAbort(wlId, '未找到清关前置公司间交易明细（贸易条款追踪记录为空或字段缺失）');
                    }
                    for (let r = 0; r < tkRows.length; r++) {
                        const row = tkRows[r];
                        const poId = row.poId;
                    ...
                    log.error('trade terms v2 qg transfer count', {
                        wlId: String(wlId || ''),
                        btnType: 'qg',
                        transferCount: list.length
                    });
                    if (list.length === 0) {
                        writeValidationErrorAndAbort(wlId, '未生成可处理的清关输入数据，请检查前置公司间交易单据和明细');
                    }
                    for (let i = 0; i < list.length; i++) {
                        const row = list[i] || {};
                        const poId = String(row.poId || '');
                        const shopId = String(row.shopId || '');
                        const subsidiaryId = String(row.sub || '');
                        const mdLocationId = String(row.md_location || '');
                        const locationAttribute = resolveLocationAttribute('qg', terms_of_trade);
                        const toLocationId = findToLocation(shopId, locationAttribute, subsidiaryId, '', 'qg', mdLocationId);
                        if (!toLocationId) {
                            const shopName = getCustomerName(shopId);
                            const subsidiaryName = getSubsidiaryName(subsidiaryId);
                            const warehouseCodeText = getWarehouseCodeText(wlId, mdLocationId);
                            writeValidationErrorAndAbort(
                                wlId,
                                '目的仓不存在，请确认。店铺:' + String(shopName || shopId || '') +
                                '，子公司:' + String(subsidiaryName || subsidiaryId || '') +
                                '，仓库属性:' + String(locationAttribute || '') +
                                '，仓库代码:' + String(warehouseCodeText || mdLocationId || '') +
                                '，前置单据:' + poId
                            );
                        }
                    }
                ...
                if (tkCount === 0) {
                    writeValidationErrorAndAbort(wlId, '未找到清关前置追踪记录，请先确认上一流程是否已完成');
                }
                var locationError = [];
                for (let i = 0; i < tkCount; i++) {
                ...
                if (list.length === 0) {
                    writeValidationErrorAndAbort(wlId, '未生成可处理的清关输入数据，请检查前置转移单和追踪记录');
                }
                return list;
    ...
    function map(context) {
        ...
        try {
            if (btnType === 'qg') {
                log.error('trade terms v2 qg transfer input line', {
                    wlId: String(wlId || ''),
                    poId: String(poId || ''),
            ...
            if (btnType === 'qg') {
                log.error('trade terms v2 qg transfer location lookup', {
                    wlId: String(wlId || ''),
                    poId: String(poId || ''),
                    shopId: String(shopId || ''),
                    store: String(value.store || ''),
                    ...
                    locationAttribute: String(locationAttribute || ''),
                    mdLocationId: String(mdLocationId || ''),
            ...
            if (!toLocationId) {
                ...
                throw error.create({
                    name: '5011',
                    message: '目的仓不存在，请确认。店铺:' + String(shopName || shopId || '') +
                        '，子公司:' + String(subsidiaryName || subsidiaryId || '') +
                        '，仓库属性:' + String(locationAttribute || '') +
                        '，仓库代码:' + String(warehouseCodeText || mdLocationId || ''),
                    notifyOff: false
            ...
            to_ids.push(toId);
            if (btnType === 'qg') {
                log.error('trade terms v2 qg transfer created', {
                    wlId: String(wlId || ''),
                    poId: String(poId || ''),
                    shopId: String(shopId || ''),
                    store: String(value.store || ''),
            ...
            if (btnType !== 'qg') {
                ...
                if (!ifId) {
                    ...
                    const irSubsidiaryId = irRec.getValue({ fieldId: 'subsidiary' }) || subsidiaryId || '';
                    const landedCostCurrencyId = getSubsidiaryCurrency(irSubsidiaryId);
                    const landedCostAgg = aggregateLinesWithCurrencyConversion(lines, cjType, btnType, landedCostCurrencyId, irRec.getValue({ fieldId: 'trandate' }) || new Date());
                    log.error('trade terms v2 transfer landed cost source', {
                        wlId: String(wlId || ''),
                        poId: String(poId || ''),
                        shopId: String(shopId || ''),
                        btnType: String(btnType || ''),
                        cjType: String(cjType || ''),
                        landedCostCurrencyId: String(landedCostCurrencyId || ''),
                        items: JSON.stringify(landedCostAgg.items || {})
                    });
                    ...
                    // 只有 wl 才写头程费用
                    if (btnType === 'wl') {
                        // if (String(cjType) === '2' && vendorId === String(CONFIG.VENDOR_ID_SKIP_TRANSFER_LANDED_COST)) {
                        //     irRec.save({ ignoreMandatoryFields: true });
                        // } else {
                        applyLandedCostPerLine(irRec, landedCostAgg.items || {}, feeMaping, cjType, btnType);
                        irRec.save({ ignoreMandatoryFields: true });
                        // }
                    } else {
                        irRec.save({ ignoreMandatoryFields: true });
            ...
            if (btnType === 'wl') {
                ...
                const tradeLinkId = resolveTradeLinkIdByShop(shopId);
                ...
                log.error('trade terms v2 tradeLinkId', {
                    wlId: String(wlId || ''),
                    btnType: String(btnType || ''),
                    configId: String(tradeLinkId || ''),
                    configName: String(executionPayload.tradeLinkName || '')
                });
                if (!isBlank(tradeLinkId)) {
                    tkRec.setValue({ fieldId: 'custrecord_wl_tk_t_jyll', value: tradeLinkId });
                }
                ...
                log.error('trade terms v2 tk jyll before save', {
                    wlId: String(wlId || ''),
                    poId: String(poId || ''),
                    shopId: String(shopId || ''),
                    jyll: String(tradeLinkId || '')
                });
                tkRec.save();
            }
    ...
    function summarize(summary) {
        ...
        if (!hasError) {
            try {
                const payloadStr = runtime.getCurrentScript().getParameter({ name: PARAM_PAYLOAD }) || '';
                const payload = parseV2Payload(payloadStr);
                const wlId = payload.wlId || '';
                const btnType = payload.btnType || '';
                const statusValue = getTkStatusValueByBtnType(btnType);
                log.error('trade terms v2 summarize status payload', {
                    wlId: String(wlId || ''),
                ...
                if (wlId && statusValue) {
                    record.submitFields({
                        ...
                        id: wlId,
                        values: (function () {
                            const values = { custrecord_swc_wl_tk_status: statusValue };
                    ...
                    log.error('trade terms v2 summarize status updated', {
                        wlId: String(wlId || ''),
                        tkStatus: String(statusValue || ''),
                        nextStep: isNaN(payload.currentStep) ? '' : String(payload.currentStep + 1)
                    });
                } else {
                    log.error('trade terms v2 summarize status skipped', {
                        wlId: String(wlId || ''),
                        currentStep: String(payload.currentStep || ''),
                        buttonId: String(payload.buttonId || ''),
                        buttonText: String(payload.buttonText || ''),
                ...
                log.error('trade terms v2 summarize status error', e);
            }
        }
    }
    ...
    function aggregateLinesWithCurrencyConversion(lines, cjType, btnType, targetCurrencyId, tranDate) {
        var feeFields = getFeeFieldsByTradeTermsAndButton(cjType, btnType);
        var convertedLines = [];
        for (var i = 0; i < (lines || []).length; i++) {
            convertedLines.push(convertLineFeesToTargetCurrency(lines[i], feeFields, targetCurrencyId, tranDate));
        }
        return aggregateLines(convertedLines, cjType, btnType);
    }

    function getFeeFieldsByTradeTermsAndButton(cjType, btnType) {
        if (cjType == '2') {
            if (btnType == 'wl') return ['trailer_fee', 'cda_fee', 'em_ffc'];
            return ['bxf_fee', 'hyf_fee', 'qgf_fee', 'jkgs_fee', 'hdf_fee', 'tcf_fee'];
        }
        if (cjType == '5') return ['bxf_fee', 'hyf_fee', 'qgf_fee', 'jkgs_fee', 'hdf_fee', 'tcf_fee', 'rkcz'];
        if (cjType == '1') {
            if (btnType == 'bg') return ['rkcz'];
            return ['trailer_fee', 'cda_fee', 'em_ffc', 'bxf_fee', 'hyf_fee', 'qgf_fee', 'jkgs_fee', 'hdf_fee', 'tcf_fee'];
        }
    ...
    function convertLineFeesToTargetCurrency(line, feeFields, targetCurrencyId, tranDate) {
        var convertedLine = {};
        for (var key in (line || {})) {
            if (Object.prototype.hasOwnProperty.call(line, key)) {
                convertedLine[key] = line[key];
            }
        }
        for (var i = 0; i < (feeFields || []).length; i++) {
            var feeField = feeFields[i];
            var feeAmount = Number(convertedLine[feeField] || 0);
            var feeCurrency = convertedLine[feeField + '_currency'] || '';
            convertedLine[feeField] = convertAmountByCurrency(feeAmount, feeCurrency, targetCurrencyId, tranDate);
        }
        return convertedLine;
    }

    function convertAmountByCurrency(amount, sourceCurrencyId, targetCurrencyId, tranDate) {
        amount = Number(amount || 0);
        if (!amount) return 0;
        if (!sourceCurrencyId || !targetCurrencyId || String(sourceCurrencyId) === String(targetCurrencyId)) {
            return amount;
        }
        return Number((amount * getStandardExchangeRate(sourceCurrencyId, targetCurrencyId, tranDate)).toFixed(2));
    }
    ...
    function getStandardExchangeRate(sourceCurrencyId, targetCurrencyId, tranDate) {
        ...
        try {
            ...
            log.error('trade terms v2 getStandardExchangeRate error', e);
            return 1;
        }
    }
    ...
    function resolveLocationAttribute(btnType, cjType) {
        const warehouseAttribute = String(executionPayload.warehouseAttribute || '');
        const startingWarehouseAttribute = String(executionPayload.startingWarehouseAttribute || '');
        if (String(cjType || '') === 'TO' && startingWarehouseAttribute) {
    ...
    function applyLandedCostPerLine(irRec, items, feeMaping, cjType, btnType) {
        ...
        if (btnType == 'wl') {
            ...
            // 国内fob   只处理 1/2/3（国内港拖车费、国内港报关代理费、国内港货代费用）
            // 海外fob  处理费用，只处理海外
            // ddp。 处理费用，全部处理
            ...
            for (let i = 0; i < lineCount; i++) {
                ...
                const quantity = Number(irRec.getSublistValue({ sublistId: 'item', fieldId: 'quantity', line: i }) || 0);
                log.error('trade terms v2 transfer landed cost ir line', {
                    btnType: String(btnType || ''),
                    cjType: String(cjType || ''),
                    line: i,
                    itemId: String(itemId || ''),
                    quantity: quantity,
                    hasItemMatch: !!(items && Object.prototype.hasOwnProperty.call(items, itemId)),
                    fee: JSON.stringify((items && items[itemId] && items[itemId].fee) || {})
                });
                if (quantity > 0 && (!items || !items.hasOwnProperty(itemId))) {
                    unmatchedItems.push(String(itemId || ''));
                    continue;
                }
                ...
                for (const feeKey in fee) {
                    ...
                    if (cjType == '2' && !fobAllowedFeeKeys[feeKey]) {
                        continue;
                    }
        ...
        if (unmatchedItems.length > 0) {
            throw new Error('到岸成本未匹配到来源费用，已中断处理。货品：' + unmatchedItems.join(','));
        }
    }

    function parseV2Payload(payloadStr) {
        let payload = {};
        try {
        ...
        const buttonText = String(payload.buttonText || payload.buttonLabel || '');
        const routeBranch = String(payload.routeBranch || '');
        ...
        return {
            ...
            btnType: resolveBtnTypeForTransfer(payload.buttonId, buttonText, routeBranch)
        };
    }
    ...
    function getCustomerName(customerId) {
        ...
        return String(s.companyname || s.altname || s.entityid || '');
    }

    function getSubsidiaryName(subsidiaryId) {
        if (isBlank(subsidiaryId)) return '';
        const s = search.lookupFields({
            type: 'subsidiary',
            id: subsidiaryId,
            columns: ['name']
    ...
    function getWarehouseCodeText(wlId, mdLocationId) {
        if (isBlank(wlId)) return '';
        const s = search.lookupFields({
            type: 'customrecord_swc_wl_plan_order',
            id: wlId,
            columns: ['custrecord_swc_md_location']
        }) || {};
        const arr = s.custrecord_swc_md_location || [];
        if (arr[0] && arr[0].text) {
            return String(arr[0].text || '');
        }
        return String(mdLocationId || '');
    }

    function resolveTradeLinkIdByShop(shopId) {
        const stepRows = executionPayload.currentStepRows || [];
        const shopSubId = String(getCustomerSubsidiary(shopId) || '');
        if (shopSubId) {
    ...
    function resolveBtnTypeForTransfer(buttonId, buttonText, routeBranch) {
