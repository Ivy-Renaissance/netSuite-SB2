...
define(['N/search', 'N/record'], function (search, record) {
    ...
    function getNeedData() {
        ...
        var infoSearchObj = search.create({
            type: 'customrecord_swc_amz_inventoryledger',
            filters:
                [
                    ['custrecord_swc_amz_eventtype', 'is', 'Receipts'],
                    'AND',
                    ['formulanumeric: NVL({custrecord_swc_amz_quantity}, 0) - NVL({custrecord_swc_amz_allocated_qty}, 0)', 'greaterthan', '0'],
                    'AND',
                    ['isinactive', 'is', 'F'],
                    'AND',
                    ['custrecord_swc_amz_sku', 'isnotempty', ''],
                    'AND',
                    ['custrecord_swc_amz_referenceid', 'isnotempty', '']
                ],
            columns:
                [
                    search.createColumn({ name: 'custrecord_swc_amz_referenceid', label: '内部编号' }),
                    search.createColumn({ name: 'custrecord_swc_amz_sku', label: 'SKU' }),
                    search.createColumn({ name: 'formulanumeric', formula: 'NVL({custrecord_swc_amz_quantity}, 0) - NVL({custrecord_swc_amz_allocated_qty}, 0)', label: '公式（数值）' }),
                    search.createColumn({ name: 'custrecord_swc_amz_quantity', label: '数量' })
                ]
        });
        ...
        if (results.length > 0) {
            for (var i = 0; i < results.length; i++) {
                ...
                var in_qty = results[i].getValue(infoSearchObj.columns[2]);
                var old_qty = results[i].getValue(infoSearchObj.columns[3]);
        ...
        search.create({
            ...
            filters:
                [
                    ...
                    'AND',
                    ['custrecord_wl_tk_t_wl_id.custrecord_swc_shipment_id', 'is', cg_num],
                    'AND',
                    ['isinactive', 'is', false]
                ],
            columns:
    ...
    function reduce(context) {
        try {
            ...
            //查询改订单所有收货的批次信息
            var rec_lot_info = getRecLotArr(to_id);
            log.debug('rec_lot_info,转移单已收货数量', rec_lot_info);
            ...
            var bill_line_count = to_data.getLineCount(sublist_id);
            if (bill_line_count > 0) {
                for (var i = bill_line_count - 1; i >= 0; i--) {
                    var line_num = to_data.getSublistValue({ sublistId: sublist_id, fieldId: 'custcol_swc_line_no', line: i });
                    to_data.selectLine(sublist_id, i);
                    to_data.setCurrentSublistValue({ sublistId: sublist_id, fieldId: 'itemreceive', value: false });
                    for (var k = 0; k < item_line.length; k++) {
                        if (item_line[k].line_no == line_num && item_line[k].allocatedQty) {
                            to_data.setCurrentSublistValue({ sublistId: sublist_id, fieldId: 'itemreceive', value: true });
                            to_data.setCurrentSublistValue({ sublistId: sublist_id, fieldId: 'quantity', value: item_line[k].allocatedQty });
                            var item_id = to_data.getCurrentSublistValue({ sublistId: sublist_id, fieldId: 'item' });
                            ...
                            if (boxSubLineCount > 0) {
                                ...
                                for (var j = 0; j < boxSubLineCount; j++) {
                                    ...
                                    var lot_id = inventorydetail.getCurrentSublistValue({ sublistId: 'inventoryassignment', fieldId: 'receiptinventorynumber' });
                                    var line_key = item_id + '_' + lot_id;
                                    var lot_qty = inventorydetail.getCurrentSublistValue({ sublistId: 'inventoryassignment', fieldId: 'quantity' }) - Number(rec_lot_info[line_key] || 0);
                                    if (Number(lot_qty > 0)) {
                                        lot_ids.push(lot_id);
                                ...
                                for (var j = boxSubLineCount - 1; j >= 0; j--) {
                                    inventorydetail.selectLine({ sublistId: 'inventoryassignment', line: j });
                                    var lot_id = inventorydetail.getCurrentSublistValue({ sublistId: 'inventoryassignment', fieldId: 'receiptinventorynumber' });
                                    if (lot_ids.indexOf(lot_id) == -1) {
                                        var line_num = inventorydetail.findSublistLineWithValue({ sublistId: 'inventoryassignment', fieldId: 'receiptinventorynumber', value: lot_id });
            ...
            if (to_data_id) {
                ...
                for (var i in batch_allocations_info) {
                    ...
                    allocation_qty_record_data.setValue('custrecord_swc_amz_list_id', i);
