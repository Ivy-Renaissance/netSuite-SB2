...
define(['N/currency', 'N/record', 'N/runtime', 'N/search'],
    (currency, record, runtime, search) => {
        ...
        const getInputData = (inputContext) => {
            var startData = runtime.getCurrentScript().getParameter({name: 'custscript_other_startdata'});
            var endData = runtime.getCurrentScript().getParameter({name: 'custscript_other_enddata'});
        ...
        const map = (mapContext) => {
            try {
                ...
                let platform;
                if (value.marketid) {
                    customerObj = getCustomer(value.marketid);
                    customer = customerObj.customer;
                    platform = customerObj.platform;
                }

                if (customer) {
                    record.submitFields({
                        type: 'customrecord_swc_settlement_report',
                        id: key,
                        values: {
                            'custrecord_swc_settlement_customer': customer || '',//店铺
                            'custrecord_swc_settlement_platform': platform || '',//平台
                        },
                    });
                }
                log.audit('写入成功',key);
        ...
        function getNeedData(startData,endData) {
            var filter = [
                ["custrecord_swc_marketid","isnotempty",""],
                "AND",
                ["custrecord_swc_settlement_customer","anyof","@NONE@"]
            ];
            if (startData) {
                filter.push('AND');
                filter.push(["custrecord_swc_marketdate","onorafter",startData]);
            }
            if (endData) {
                filter.push('AND');
                filter.push(["custrecord_swc_marketdate","onorbefore",endData]);
            }
            const customrecord_swc_amazon_v2SearchObj = search.create({
                ...
                columns:
                    [
                        ...
                        search.createColumn({name: "custrecord_swc_marketid", label: "站点ID"})
                    ]
            });
            ...
            results.forEach(value => {
                obj[value.id] = {
                    'marketid': value.getValue({name: "custrecord_swc_marketid", label: "站点ID"})
                };
            })
        ...
        function getCustomer(marketid) {
            const customerSearchObj = search.create({
                ...
                columns:
                    [
                        search.createColumn({name: "internalid", label: "内部 ID"}),
                        search.createColumn({name: "custentity_swc_platform", label: "平台"})
                    ]
            });
            var obj = {};
            customerSearchObj.run().each(function(result){
                obj.customer = result.getValue({name: "internalid", label: "内部 ID"});
                obj.platform = result.getValue({name: "custentity_swc_platform", label: "平台"});
                return true;
            });
