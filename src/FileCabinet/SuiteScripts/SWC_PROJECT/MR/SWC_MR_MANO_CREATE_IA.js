...
define(['N/search', 'N/record', 'N/error', 'N/format', '../common/moment', '../common/SWC_CONFIG_DATA'], function (search, record, error, format, moment, SWC_CONFIG_DATA) {
    ...
    function getNeedData() {
        ...
        search.create({
            ...
            filters: [
                ['custrecord_swc_mano_inventoryadjustment', 'anyof', '@NONE@'],
                'AND',
                ['isinactive', 'is', false],
                'AND',
                ['custrecord_swc_mano_location', 'noneof', '@NONE@'],
                'AND',
                ['custrecord_swc_mano_type', 'is', 'Inventory adjustment'],
                'AND',
                ['custrecord_swc_mano_sku', 'isnotempty', ''],
                'AND',
                ['internalid', 'anyof', '513']
            ],
            columns: [
    ...
    function map(context) {
        ...
        try {
            ...
            inv_rec.setValue({ fieldId: 'custbody_swc_gl_mamo_inventoryledger', value: roc_id }); // 关联发货报告
            ...
            if (inv_rec_id) {
                ...
                record.submitFields({
                    ...
                    values: {
                        ...
                        custrecord_swc_mano_inventoryadjustment: inv_rec_id
                    }
                });
            }
        } catch (e) {
