 ...
 */
define(['N/record', 'N/runtime', 'N/search', 'N/https', 'N/config', '../common/moment'],
    /**
 * @param{record} record
 * @param{runtime} runtime
 * @param{search} search
 */
    (record, runtime, search, https, config, moment) => {
        /**
        ...
        const getInputData = (inputContext) => {
            // var demandRec = record.load({ type: 'customrecord_swc_demand_plan', id: 1 });
            // var xz = demandRec.getValue('custrecord_swc_dp_special_modification')
            // var xz1 = demandRec.getValue('custrecord_swc_dp_correction1')
            // var xz2 = demandRec.getValue('custrecord_swc_dp_correction2')
            // var xz3 = demandRec.getValue('custrecord_swc_dp_correction3')
            // log.debug('xz', xz)
            ...
            // return []
            var _batch = runtime.getCurrentScript().getParameter('custscript_swc_dp_batch');//备货批次
            ...
            var mySearch = search.create({
                ...
                filters: [
                    ...
                    { name: 'custrecord_swc_sf_batch', operator: search.Operator.ANYOF, values: _batch },//
            ...
            var mySearch = search.create({
                ...
                filters: [
                    { name: 'internalid', operator: search.Operator.ANYOF, values: ['6900', '1032', '6482'] },//给定测试条目 6482
        ...
        const map = (mapContext) => {
            try {
                ...
                search.create({
                    ...
                    columns: [
                        //备货维度支持的仓库类型，重要说明：备货维度支持多种仓库类型，但是有可能该备货维度下的店铺只有某些店铺支持其中某类仓库类型，所以计算销量时要先搜索对应仓库类型下的店铺
                        { name: 'custentity_swc_location_type' },
                ...
                for (let index = 0; index < locationTypes.length; index++) {
                    ...
                    search.create({
                        ...
                        filters: [
                            ...
                            { name: 'custrecord_swc_dp_batch', operator: search.Operator.IS, values: _batch }//常规备货
                        ],
                        columns: [
                        ...
                        exist = true;
                        demandRec = record.load({ type: 'customrecord_swc_demand_plan', id: rec.id });
                        return false;
                    });
                    if (!exist) {
                        demandRec = record.create({ type: 'customrecord_swc_demand_plan' });
                    }

                    // demandRec.setValue({ fieldId: 'name', value: 1 }) //备货批次编码
                    demandRec.setValue({ fieldId: 'custrecord_swc_dp_batch_month', value: "BH-" + moment().add(16, 'hours').startOf('month').format('YYYYMM') }) //备货月批次编码
                    demandRec.setValue({ fieldId: 'custrecord_swc_dp_batch', value: _batch }) //备货批次,默认常规备货
                    demandRec.setValue({ fieldId: 'custrecord_swc_dp_store', value: store })//平台（店铺）
                    demandRec.setValue({ fieldId: 'custrecord_swc_dp_country', value: country })//国家编码
                    demandRec.setValue({ fieldId: 'custrecord_swc_dp_location_type', value: locationType })//仓库类型
                    demandRec.setValue({ fieldId: 'custrecord_swc_dp_sku', value: sku })//仓库类型
                    demandRec.setValue({ fieldId: 'custrecord_swc_dp_applicant_date', value: moment().add(16, 'hours').toDate() })//申请日期
                    // demandRec.setText({ fieldId: 'custrecord_swc_dp_applicant_date', text: '2026-04-03 2:50:40 PM' })//申请日期

                    // demandRec.setText({ fieldId: 'custrecord_swc_dp_applicant', text: obj.salesperson })//销售人员要取自SKU映射表
                    var level = getSKULevel(sku, country);
                    if (level) {
                        demandRec.setValue({ fieldId: 'custrecord_swc_dp_sku_level', value: level.level })//产品等级  
                    }
                    var totalMonths = runtime.getCurrentScript().getParameter('custscript_swc_dp_totalmonths');//计算总月份取配置值
                    demandRec.setValue({ fieldId: 'custrecord_swc_dp_months', value: totalMonths })//预测的总月份，默认6个月，可配置8个月
                    ...
                    search.create({
                        ...
                        leadTime = rec.getValue(rec.columns[0]) * 1 + rec.getValue(rec.columns[1]) * 1 + level.safeDate * 1
                        leadTimeFound = true
                        return true;
                    });
                    if (totalMonths == 8) {
                    ...
                    search.create({
                        ...
                        filters: [
                            [
                                ...
                                ['custrecord_swc_sf_store', 'anyof', stores], 'and',
                                ['custrecord_swc_sf_country', 'is', country], 'and',
                                ['custrecord_swc_sf_batch', 'is', _batch], 'and',
                        ...
                        if (itemId == sku) {//套装下可能会搜出其他子件SKU的需求数，这里只取对应SKU的
                            ...
                            demandRec.setValue({ fieldId: 'custrecord_swc_dp_forcast_m1', value: unit_m1 })//第1月需求
                            ...
                            demandRec.setValue({ fieldId: 'custrecord_swc_dp_forcast_m2', value: unit_m2 })//第2月需求 
                            ...
                            demandRec.setValue({ fieldId: 'custrecord_swc_dp_forcast_m3', value: unit_m3 })//第3月需求 
                            ...
                            demandRec.setValue({ fieldId: 'custrecord_swc_dp_forcast_m4', value: unit_m4 })//第4月需求 
                            ...
                            demandRec.setValue({ fieldId: 'custrecord_swc_dp_forcast_m5', value: unit_m5 })//第5月需求 
                            ...
                            demandRec.setValue({ fieldId: 'custrecord_swc_dp_forcast_m6', value: unit_m6 })//第6月需求 
                            ...
                            demandRec.setValue({ fieldId: 'custrecord_swc_dp_forcast_m7', value: unit_m7 })//第7月需求 
                            ...
                            demandRec.setValue({ fieldId: 'custrecord_swc_dp_forcast_m8', value: unit_m8 })//第8月需求 
                    ...
                    demandRec.setValue({ fieldId: 'custrecord_swc_dp_inventory_status', value: 3 })//查不到去化率时默认设置无标准销量
                    search.create({
                    ...
                    search.create({
                        ...
                        // [
                        //     { name: 'formulanumeric', formula: "case when {location.custrecord_swc_us_districts} = '1' then {quantity}*{item.memberquantity} else 0 end", summary: 'SUM' },//美西销量
                        //     { name: 'formulanumeric', formula: "case when {location.custrecord_swc_us_districts} = '2' then {quantity}*{item.memberquantity} else 0 end", summary: 'SUM' },//美栋销量
                        //     { name: 'formulanumeric', formula: "case when {location.custrecord_swc_us_districts} = '3' then {quantity}*{item.memberquantity} else 0 end", summary: 'SUM' },//美南销量
                        //     { name: 'formulanumeric', formula: "case when {location.custrecord_swc_us_districts} = '4' then {quantity}*{item.memberquantity} else 0 end", summary: 'SUM' },//美东南销量
                        //     { name: 'formulanumeric', formula: "case when {location.custrecord_swc_us_districts} = '5' then {quantity}*{item.memberquantity} else 0 end", summary: 'SUM' },//美西南销量
                        //     { name: 'formulanumeric', formula: "case when {location.custrecord_swc_us_districts} = '6' then {quantity}*{item.memberquantity} else 0 end", summary: 'SUM' },//加东销量
                        //     { name: 'formulanumeric', formula: "case when {location.custrecord_swc_us_districts} = '7' then {quantity}*{item.memberquantity} else 0 end", summary: 'SUM' }//加西销量
        ...
        const reduce = (reduceContext) => {
            try {
                ...
                // 1.1
                // 1.美东，美东南，美西南，美中都超期 2.美东，美东南，美西南，美中都不超期

                //美西数量=PMC数量*35%
                // ROUND({custrecord_swc_dp_quantity}*0.35)

                // 美东南 =（PMC数量 - 美西数量）* 美东南订单 /（美东南订单 + 美东订单 + 美中订单 + 美西南订单）
                // 美东 =（PMC数量 - 美西数量）* 美东订单 /（美东南订单 + 美东订单 + 美中订单 + 美西南订单）
                // 美西南 =（PMC数量 - 美西数量）* 美西南订单 /（美东南订单 + 美东订单 + 美中订单 + 美西南订单）
                // 美中 = PMC数量 - 美西 - 美东南 - 美东 - 美西南
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) ELSE  Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mdnxssl}/{custrecord_swc_dp_us_xssl}) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) ELSE  Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl}) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) ELSE  Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mdxssl}/{custrecord_swc_dp_us_xssl}) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN {custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35)- Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4)*3 ELSE {custrecord_swc_dp_quantity}- ROUND({custrecord_swc_dp_quantity}*0.35) - Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mdnxssl}/{custrecord_swc_dp_us_xssl}) - Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl}) - Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mdxssl}/{custrecord_swc_dp_us_xssl}) END


                // 1.2 美东，美西南，美东南美中四个中有超期
                //美西数量=PMC数量*35%
                // ROUND({custrecord_swc_dp_quantity}*0.35)
                // 美东 = (PMC数量 - 美西数量) * 美东订单 /（美东南订单 + 美东订单 + 美中订单 + 美西南订单）-美东超龄数量，如果小于0或者等于0，则 = 0
                // 美东南 = (PMC数量 - 美西数量) * 美东南订单 /（美东南订单 + 美东订单 + 美中订单 + 美西南订单）-美东南超龄数量，如果小于0或者等于0，则 = 0
                // 美西南 = (PMC数量 - 美西数量) * 美西南订单 /（美东南订单 + 美东订单 + 美中订单 + 美西南订单）-美西南超龄数量，如果小于0或者等于0，则 = 0
                // 美中 = PMC数量 - 美西 - 美东南 - 美东 - 美西南
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) - {custrecord_swc_dp_mdncqsl},0)  ELSE  GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mdnxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mdncqsl},0) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) - {custrecord_swc_dp_mxncqsl},0) ELSE  GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mxncqsl},0) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) - {custrecord_swc_dp_mdcqsl},0) ELSE  GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mdxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mdcqsl},0) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN {custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35)- GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) - {custrecord_swc_dp_mdncqsl},0) - GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) - {custrecord_swc_dp_mxncqsl},0) - GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) - {custrecord_swc_dp_mdcqsl},0) ELSE {custrecord_swc_dp_quantity}- ROUND({custrecord_swc_dp_quantity}*0.35) - GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mdnxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mdncqsl},0) - GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mdncqsl},0)  - GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mdxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mdncqsl},0) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN {custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35)- GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) - {custrecord_swc_dp_mdncqsl},0) - GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) - {custrecord_swc_dp_mxncqsl},0) - GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))/4) - {custrecord_swc_dp_mdcqsl},0) ELSE {custrecord_swc_dp_quantity}- ROUND({custrecord_swc_dp_quantity}*0.35) - GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mdnxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mdncqsl},0) - GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mdncqsl},0)  - GREATEST(Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*0.35))*{custrecord_swc_dp_mdxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mdncqsl},0)


                // 第二部分，库存状态正常and五个区域都有超期大于0
                //美西=PMC数量*美西订单比例-美西超龄数量，如果小于0或者等于0，则=0
                // CASE WHEN {custrecord_swc_dp_us_xssl} = 0 THEN GREATEST(Round({custrecord_swc_dp_quantity} / 5) - {custrecord_swc_dp_mxcqsl}, 0) ELSE GREATEST(Round({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) - {custrecord_swc_dp_mxcqsl}), 0) END
                // 美东南=（PMC数量-美西数量）*美东南订单/（美东南订单+美东订单+美中订单+美西南订单）
                // 美东=（PMC数量-美西数量）*美东订单/（美东南订单+美东订单+美中订单+美西南订单）
                // 美西南=（PMC数量-美西数量）*美西南订单/（美东南订单+美东订单+美中订单+美西南订单）
                // 美中=PMC数量-美西-美东南-美东-美西南
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - GREATEST(Round({custrecord_swc_dp_quantity}/5) - {custrecord_swc_dp_mxcqsl},0))/4) ELSE  Round(({custrecord_swc_dp_quantity} - GREATEST(Round({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) - {custrecord_swc_dp_mxcqsl}), 0))*{custrecord_swc_dp_mdnxssl}/{custrecord_swc_dp_us_xssl}) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - GREATEST(Round({custrecord_swc_dp_quantity}/5) - {custrecord_swc_dp_mxcqsl},0))/4) ELSE  Round(({custrecord_swc_dp_quantity} - GREATEST(Round({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) - {custrecord_swc_dp_mxcqsl}), 0))*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl}) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - GREATEST(Round({custrecord_swc_dp_quantity}/5) - {custrecord_swc_dp_mxcqsl},0))/4) ELSE  Round(({custrecord_swc_dp_quantity} - GREATEST(Round({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) - {custrecord_swc_dp_mxcqsl}), 0))*{custrecord_swc_dp_mdxssl}/{custrecord_swc_dp_us_xssl}) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN {custrecord_swc_dp_quantity} - GREATEST(Round({custrecord_swc_dp_quantity}/5) - {custrecord_swc_dp_mxcqsl},0) - Round(({custrecord_swc_dp_quantity} - GREATEST(Round({custrecord_swc_dp_quantity}/5) - {custrecord_swc_dp_mxcqsl},0))/4)*3 ELSE {custrecord_swc_dp_quantity}- GREATEST(Round({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) - {custrecord_swc_dp_mxcqsl}), 0) - Round(({custrecord_swc_dp_quantity} - GREATEST(Round({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) - {custrecord_swc_dp_mxcqsl}), 0))*{custrecord_swc_dp_mdnxssl}/{custrecord_swc_dp_us_xssl}) - Round(({custrecord_swc_dp_quantity} - GREATEST(Round({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) - {custrecord_swc_dp_mxcqsl}), 0))*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl}) - Round(({custrecord_swc_dp_quantity} - GREATEST(Round({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) - {custrecord_swc_dp_mxcqsl}), 0))*{custrecord_swc_dp_mdxssl}/{custrecord_swc_dp_us_xssl}) END


                // 美西=PMC数量*美西订单/总订单
                // ROUND({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}))
                // 美东南=（PMC数量-美西数量）*美东南订单/（美东南订单+美东订单+美中订单+美西南订单）
                // 美东=（PMC数量-美西数量）*美东订单/（美东南订单+美东订单+美中订单+美西南订单）
                // 美西南=（PMC数量-美西数量）*美西南订单/（美东南订单+美东订单+美中订单+美西南订单）
                // 美中=PMC数量-美西-美东南-美东-美西南
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl})))/4) ELSE  Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl})))*{custrecord_swc_dp_mdnxssl}/{custrecord_swc_dp_us_xssl}) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl})))/4) ELSE  Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl})))*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl}) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl})))/4) ELSE  Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl})))*{custrecord_swc_dp_mdxssl}/{custrecord_swc_dp_us_xssl}) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN {custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl}))- Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl})))/4)*3 ELSE {custrecord_swc_dp_quantity}- ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl})) - Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl})))*{custrecord_swc_dp_mdnxssl}/{custrecord_swc_dp_us_xssl}) - Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl})))*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl}) - Round(({custrecord_swc_dp_quantity} - ROUND({custrecord_swc_dp_quantity}*{custrecord_swc_dp_mxxssl}/({custrecord_swc_dp_mxxssl}+{custrecord_swc_dp_us_xssl})))*{custrecord_swc_dp_mdxssl}/{custrecord_swc_dp_us_xssl}) END
                ...
                // ROUND({custrecord_swc_dp_quantity}*0.35)
                // 美西=PMC数量*美西订单/总订单
                // CASE  WHEN ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) = 0 THEN ROUND({custrecord_swc_dp_quantity} / 5) ELSE ROUND({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl})) END
                // 美西=PMC数量*美西订单/（总订单数量）-美西超龄数量 ，如果小于0或者等于0，则=0
                // CASE WHEN ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) = 0 THEN GREATEST(Round({custrecord_swc_dp_quantity} / 5) - {custrecord_swc_dp_mxcqsl}, 0) ELSE GREATEST(Round({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) - {custrecord_swc_dp_mxcqsl}), 0) END
                ...
                // 美西南=（PMC数量-美西数量）*美西南订单/（美东南订单+美东订单+美中订单+美西南订单）
                // 美中=PMC数量-美西-美东南-美东-美西南
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})/4) ELSE  Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})*{custrecord_swc_dp_mdnxssl}/{custrecord_swc_dp_us_xssl}) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})/4) ELSE  Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl}) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})/4) ELSE  Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})*{custrecord_swc_dp_mdxssl}/{custrecord_swc_dp_us_xssl}) END
                // {custrecord_swc_dp_quantity}- {custrecord_swc_dp_us_west_qty} - {custrecord_swc_dp_us_southeast_qty} - {custrecord_swc_dp_us_east_qty} - {custrecord_swc_dp_us_southwest_qty}

                // 美东 = (PMC数量 - 美西数量) * 美东订单 /（美东南订单 + 美东订单 + 美中订单 + 美西南订单）-美东超龄数量，如果小于0或者等于0，则 = 0
                // 美东南 = (PMC数量 - 美西数量) * 美东南订单 /（美东南订单 + 美东订单 + 美中订单 + 美西南订单）-美东南超龄数量，如果小于0或者等于0，则 = 0
                // 美西南 = (PMC数量 - 美西数量) * 美西南订单 /（美东南订单 + 美东订单 + 美中订单 + 美西南订单）-美西南超龄数量，如果小于0或者等于0，则 = 0
                // 美中 = PMC数量 - 美西 - 美东南 - 美东 - 美西南
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN GREATEST(Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})/4) - {custrecord_swc_dp_mdncqsl},0)  ELSE  GREATEST(Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})*{custrecord_swc_dp_mdnxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mdncqsl},0) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN GREATEST(Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})/4) - {custrecord_swc_dp_mxncqsl},0)  ELSE  GREATEST(Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mxncqsl},0) END
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN GREATEST(Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})/4) - {custrecord_swc_dp_mdcqsl},0)  ELSE  GREATEST(Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})*{custrecord_swc_dp_mdxssl}/{custrecord_swc_dp_us_xssl})- {custrecord_swc_dp_mdcqsl},0) END
                // {custrecord_swc_dp_quantity}- {custrecord_swc_dp_us_west_qty} - {custrecord_swc_dp_us_southeast_qty} - {custrecord_swc_dp_us_east_qty} - {custrecord_swc_dp_us_southwest_qty}


                // CASE  WHEN ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) = 0 THEN ROUND({custrecord_swc_dp_quantity} / 5) ELSE ROUND({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mdnxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl})) END
                // CASE  WHEN ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) = 0 THEN ROUND({custrecord_swc_dp_quantity} / 5) ELSE ROUND({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxnxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl})) END
                // CASE  WHEN ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) = 0 THEN ROUND({custrecord_swc_dp_quantity} / 5) ELSE ROUND({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mdxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl})) END
                // {custrecord_swc_dp_quantity}- {custrecord_swc_dp_us_west_qty} - {custrecord_swc_dp_us_southeast_qty} - {custrecord_swc_dp_us_east_qty} - {custrecord_swc_dp_us_southwest_qty}
                ...
                //分三区逻辑
                // 美西1=分0.4
                // ROUND({custrecord_swc_dp_quantity}*0.4)
                // 美西2=PMC审批备货*美西订单/总订单-美西超龄数量
                // CASE WHEN ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) = 0 THEN GREATEST(Round({custrecord_swc_dp_quantity} / 3) - {custrecord_swc_dp_mxcqsl}, 0) ELSE GREATEST(Round({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) - {custrecord_swc_dp_mxcqsl}), 0) END
                // 美西3=PMC审批备货*美西订单/总订单
                // CASE WHEN ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl}) = 0 THEN GREATEST(Round({custrecord_swc_dp_quantity} / 3), 0) ELSE GREATEST(Round({custrecord_swc_dp_quantity} * {custrecord_swc_dp_mxxssl} / ({custrecord_swc_dp_mxxssl} + {custrecord_swc_dp_us_xssl})), 0) END

                // 美西南数量1 = (PMC审批备货-美西)*美南订单/(美南订单+美中订单)
                // CASE  WHEN ({custrecord_swc_dp_mxnxssl}+{custrecord_swc_dp_mzxssl}) = 0 THEN Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})/2) ELSE  Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})*{custrecord_swc_dp_mxnxssl}/({custrecord_swc_dp_mxnxssl}+{custrecord_swc_dp_mzxssl})) END
                // 美中数量1 = PMC审批备货-美西-美南
                // {custrecord_swc_dp_quantity}- {custrecord_swc_dp_us_west_qty} -  {custrecord_swc_dp_us_southwest_qty}

                //美西南数量2 = (PMC审批备货-美西)*美南订单/(美南订单+美中订单)
                // CASE  WHEN {custrecord_swc_dp_us_xssl} = 0 THEN Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})/2) ELSE  Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty})*{custrecord_swc_dp_mxnxssl}/{custrecord_swc_dp_us_xssl}) END
                // 美西南3 = (PMC审批备货-美西)*美南订单/(美南订单+美中订单)-美南超龄数量
                // CASE WHEN {custrecord_swc_dp_us_xssl} = 0 THEN GREATEST(Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty}) / 2) - {custrecord_swc_dp_mxcqsl}, 0) ELSE GREATEST(Round(({custrecord_swc_dp_quantity} - {custrecord_swc_dp_us_west_qty}) * {custrecord_swc_dp_mxnxssl} /{custrecord_swc_dp_us_xssl} - {custrecord_swc_dp_mxncqsl}), 0) END
