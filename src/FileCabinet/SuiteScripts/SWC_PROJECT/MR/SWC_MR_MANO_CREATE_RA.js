...
define(['N/search', 'N/record', 'N/error', 'N/format', '../common/moment'], function (search, record, error, format, moment) {
    ...
    function getNeedData() {
        ...
        search.create({
            type: 'customrecord_swc_mamo_inventoryledger',
            filters: [
                ['custrecord_swc_mano_roc_relation_ra', 'anyof', '@NONE@'],
                'AND',
                ['isinactive', 'is', false],
                'AND',
                ['custrecord_swc_mano_location', 'noneof', '@NONE@'],
                'AND',
                ['custrecord_swc_mano_reference', 'isnotempty', ''],
                'AND',
                ['custrecord_swc_mano_type', 'is', 'Customer Return'],
                'AND',
                ['custrecord_swc_mano_sku', 'isnotempty', ''],
                'AND',
                ['internalid', 'anyof', '1405']
            ],
            columns: [
                ...
                { name: 'custrecord_swc_mano_units' }
            ]
        }).run().each(function (rec) {
            need_rec.push({
                id: rec.id,
                retry: rec.getValue({ name: 'custrecord_swc_mano_retry', sort: search.Sort.ASC }),
                return_date: rec.getValue({ name: 'custrecord_swc_mano_created', sort: search.Sort.ASC }),
                acc_id: rec.getValue('custrecord_swc_mano_location'),
                orderid: rec.getValue('custrecord_swc_mano_reference'),
                orderitem: rec.getValue('custrecord_swc_mano_sku'),
                orderitemQty: rec.getValue('custrecord_swc_mano_units')
            })
            return --limit > 0
        })
        return need_rec;
    ...
    function map(context) {
        ...
        try {
            ...
            if (Object.keys(so_info).length > 0) {
                ...
                rec.setValue({ fieldId: 'custbody_swc_gl_mamo_inventoryledger', value: roc_id });
                ...
                if (Object.keys(item_price).length > 0) {
                    rec.setCurrentSublistValue({ sublistId: 'item', fieldId: 'rate', value: item_price[item_id]['item_price'] || '' });
                }
                rec.commitLine({ sublistId: 'item' });
                ra_id = rec.save({ ignoreMandatoryFields: true });
            }
            log.debug('ra_id', '退货授权已创建');
            if (ra_id) {
                //创建货品收据
                ...
                ir_rec.setValue({ fieldId: 'custbody_swc_gl_mamo_inventoryledger', value: roc_id });
                ...
                if (ir_id) {
                    ...
                    // cm_rec.setValue({ fieldId: 'custbody_swc_gl_mamo_inventoryledger', value: roc_id });
                    ...
                    record.submitFields({
                        ...
                        values: {
                            ...
                            custrecord_swc_mano_roc_relation_ra: ra_id,
                            custrecord_swc_mano_roc_relation_ir: ir_id,
                            // custrecord_swc_mano_roc_relation_cm: cm_id,
                        }
                    });
                    // }
                }
            ...
            record.submitFields({
                ...
                id: roc_id,
                values: {
                    custrecord_swc_mano_retry: Number(retry) + 1,
                    custrecord_swc_mano_error: e.message,
