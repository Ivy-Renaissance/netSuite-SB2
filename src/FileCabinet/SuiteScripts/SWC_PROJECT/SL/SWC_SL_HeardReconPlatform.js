...
define(["N/ui/serverWidget","N/runtime","N/record",'N/task',"../common/SWC_Translate","../APP/SWC_APP_HeardReconPlatform"],
    ...
    (serverWidget,runtime,record,task,SWC_Translate,app) => {
        ...
        function crtForm() {
            // 创建表单对象
            var form = serverWidget.createForm({title: "物流对账平台"});
            // 设置客户端脚本
            form.clientScriptModulePath = "../CS/SWC_CS_HeardReconPlatform";
            // 提交按钮设置为刷新
        ...
        function createReconciliation(adData) {
            ...
            mrTask.scriptId = 'customscript_swc_mr_heardrecon';
            //设置部署ID
            mrTask.deploymentId = 'customdeploy_swc_mr_heardrecon';

            log.audit('adData',adData);
            //传递参数给 Map/Reduce 脚本
            mrTask.params = {
                // 这些参数可以在 Map/Reduce 脚本的 context.param1, context.param2 中获取
                'custscript_heardreconc_json': JSON.stringify(adData),
            };
