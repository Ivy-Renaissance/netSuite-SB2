...
| 控件 | 控件 ID |
|---|---|
| 申请人 | `widget17799352112200001` |
| 申请部门 | `widget17799352332450001` |
| 单据ID（显示 `PREPAYxxxxx`） | `widget17799337158960001` |
| 单据内部 ID（数字） | `widget17799345058410001` |
| 金额 | `widget17799345090550001` |
| 日期 | `widget17799345126080001` |
| 明细 | `widget17799345168640001` |
| 明细文本 1 | `widget17799349566510001` |
| 明细文本 2 | `widget17799349594420001` |
| 明细备注 | `widget17799349691000001` |
| 明细数字 | `widget17799349759800001` |
| 明细金额 | `widget17799349784270001` |

如果飞书审批模板重新发布后控件 ID 变化，需要同步更新配置文件中的 `FEISHU_WIDGET`。
...
飞书应用权限需要包含审批实例查询、审批任务同意、指定退回等审批任务操作权限；权限调整后需重新发布应用。
...
| 飞书创建实例失败 | App 权限、审批 code、控件 ID 或发起人异常 | 查看 UE 执行日志中的飞书返回 body |
...
- [ ] 3 个脚本文件已上传到正确目录。
- [ ] 公共配置中的字段 ID、状态内部 ID、飞书控件 ID、飞书节点 ID 已确认。
